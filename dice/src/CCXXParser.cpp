/**
 *	\file	dice/src/CCXXParser.cpp
 *	\brief	contains the implementation of the class CCXXParser
 *
...
extern FILE *gcc_cxxin;
extern int gcc_cxxdebug;
extern int gcc_cxx_flex_debug;
extern int gcc_cxxparse();
//@}
 ...
 */
unsigned char CCXXParser::Import(String sFilename)
{
    // 1. save state
	// - scanner's buffer
	// - line number
	...
	// - sTopLevelInFileName
	// - sFilename
	m_pBuffer = GetCurrentBufferGccCxx();
	...
	{
	    Verbose("Cxx::Import(%s) requires a new parser (%d)\n", (const char*)sFilename, nFileType);
		// 3. create new Parser
		...
		RestoreBufferGccCxx(m_pBuffer, false);
	}
	else
	{
	    m_nFiles++;
	    Verbose("Cxx::Import(%s) does not require a new parser, increment file count to %d\n",
 ...
 */
bool CCXXParser::Parse(void *scan_buffer, String sFilename, int nIDL, bool bVerbose, bool bPreProcessOnly)
{
//     TRACE("Cxx::Parse(%p, %s, %d, %s, %s) called\n", scan_buffer,
...
// 		bPreProcessOnly?"true":"false");
	...
	if (!sPath.IsEmpty() && sOrigName.IsEmpty())
		sOrigName = sFilename.Right(sFilename.GetLength()-sPath.GetLength());
	...
	{
		gcc_cxxdebug = 1;
		gcc_cxx_flex_debug = 1;
	}
    else
	{
		gcc_cxxdebug = 0;
		gcc_cxx_flex_debug = 0;
	}

	if (!scan_buffer)
	{
		gcc_cxxin = fInput;
		StartBufferGccCxx(fInput);
    }
	else
		RestoreBufferGccCxx(scan_buffer, true);
