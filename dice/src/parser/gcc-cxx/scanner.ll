...
<line>{Filename} {
					BEGIN(line2);
   					...
   					if (strlen (yytext) > 2)
   					{
    					...
    					sNewFileNameGccCxx.TrimLeft('"');
    					sNewFileNameGccCxx.TrimRight('"');
   					}
   					else
...
<line2>[1-4]    {
					...
					if (gcc_cxxdebug)
					    fprintf(stderr, "Cxx: # %d \"%s\" %d found\n", nNewLineNumberGccCxx,
						    (const char*)sNewFileNameGccCxx, nFlags);
					switch(nFlags)
					{
					case 1:
						{
							...
							case 1:
							    // ok, but create file
							    {
									...
									if (!sPath.IsEmpty() && sOrigName.IsEmpty())
										sOrigName = sNewFileNameGccCxx.Right(sNewFileNameGccCxx.GetLength()-sPath.GetLength());
