...
<string><<EOF>>			{
                            ...
                            fprintf(stderr, "%s:%d: Unterminated string at end of file. Could have started here: %d.\n",
								(const char*)sInFileName, gLineNumber, nStartOfStringInc);
							yyterminate();
						}
...
<<EOF>>				{
						...
						if (state_stack_size > 0)
						{
							...
							// statement
							fprintf(yyout, "# %d \"%s\" 2", gLineNumber+1, (const char*)sInFileName);
						}
						else
							yyterminate();
                    }
