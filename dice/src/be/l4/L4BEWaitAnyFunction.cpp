...
{
    IMPLEMENT_DYNAMIC_BASE(CL4BEWaitAnyFunction, CBEWaitAnyFunction);
}
 ...
 */
void CL4BEWaitAnyFunction::WriteUnmarshalling(CBEFile * pFile, int nStartOffset, bool& bUseConstOffset, CBEContext * pContext)
{
 ...
 */
void CL4BEWaitAnyFunction::WriteVariableDeclaration(CBEFile * pFile, CBEContext * pContext)
{
 ...
 * number of flexpages.
 */
void CL4BEWaitAnyFunction::WriteFlexpageOpcodePatch(CBEFile *pFile, CBEContext *pContext)
{
    if (!(((CL4BEMsgBufferType*)m_pMsgBuffer)->HasReceiveFlexpages()))
        return;
    ...
    else
    {
        ...
        String sBuffer = pContext->GetNameFactory()->GetMessageBufferVariable(pContext);
 ...
 *  \param pContext the context of the write operation
 */
void CL4BEWaitAnyFunction::WriteVariableInitialization(CBEFile * pFile, CBEContext * pContext)
{
    // call base class
