...
vm_offset_t error_task_stack_base = 0;
...
void sit_and_wait(int exp)
{
  ...
  l4_i386_ipc_receive(L4_NIL_ID, 0, &ignore, &ignore, t , &dope);
}
...
void wait_forever(void)
{
  ...
  l4_i386_ipc_receive(L4_NIL_ID, 0, &ignore, &ignore, L4_IPC_NEVER , &dope);
}
     

bool get_page_at(dword_t mapaddr, l4_threadid_t sigma0_id)
{
  l4_snd_fpage_t fp;
  l4_threadid_t pager;
  l4_msgdope_t dope;
  ...
  if(l4_is_invalid_id(sigma0_id))
    get_thread_ids(NULL, NULL, &pager, NULL);
  else
    pager = sigma0_id;

  res = l4_i386_ipc_call(pager,			     // dest
...
const unsigned stack_size = PAGE_SIZE;
char stack2[max_stack * stack_size];

/* no static initialization at the moment */
vm_offset_t stack_pointer = 0;

vm_offset_t get_next_stack(void)
{
  if(stack_pointer <= reinterpret_cast<vm_offset_t>(stack2) + stack_size)
    {
      printf("Assertion stack pointer underflow\n");
      printf("stack2 %08x stack_pointer %08x\n",
	     reinterpret_cast<vm_offset_t>(stack2), stack_pointer);
      wait_forever();
    }

  vm_offset_t res = stack_pointer;
...
void hello(char * id)
{
  ...
  printf("This is %s, task %u, thread %u, "
	 "preempter %u(%u), pager %u(%u), stack %08x\n",
	 id,
...
void send_to_controller(controller_message_t w0, dword_t w1)
{
  int res;
  l4_msgdope_t dope;
  /* give controller a short time */
  l4_timeout_t t = L4_IPC_TIMEOUT(255,10,255,10,0,0);

  res = l4_i386_ipc_send(controller_thread_id,
			 L4_IPC_SHORT_MSG,
			 w0, w1,
			 t,
			 &dope);
  ...
  if(res == L4_IPC_SETIMEOUT || res == L4_IPC_RESNDPFTO)
    printf("controller timeout with %d(%s) (%08x)\n", 
...
void receive(controller_message_t * code, dword_t * w1)
{
  int res;
  l4_msgdope_t dope;
  l4_threadid_t src;
...
void receive_no_info(controller_message_t *code, dword_t *w1)
{
  do
    receive(code, w1);
...
void controller_check(controller_message_t code, dword_t w1, bool b)
{
  if(! b)
    {
      sit_and_wait(9);
      printf("controller got unexpected %d(%s) (%08x)\n", code, 
...
void idt_thread(void)
{
  ...
  /* get first int */
  res = l4_i386_ipc_wait(&src,			 /* source */
  ...
  while(1)
    {
      ...
      //     (* reinterpret_cast<dword_t *>(regs.eip +3)));
      

      if( ! handle_interrupt() )
	{
	  printf("Int %ld, error %08lx in thread %d(%d) at EIP %08lx\n",
      ...
      res = l4_i386_ipc_reply_and_wait
	(src,					 /* destination */
	 L4_IPC_SHORT_MSG,			 /* snd desc */
	 0,					 /* snd data 0 */
	 0 ,					 /* snd data 1 */
...
static void pager_thread(void)
{
  ...
  dword_t fault_address, fault_eip, ignore;
  l4_snd_fpage_t fp;
  ...
  while(1)
    {
      ...
      printf("[%s page fault in %d(%d) at %08x (EIP %08x) ",
      ...
      else if( l4_is_io_page_fault(fault_address))
	{
	  ...
	  send_to_controller(IO_FAULT, fp.fpage.fpage);
				...
				/* in Code ? */
      else if((dword_t) & _start <= fault_address 
	 && ((dword_t) & _edata > fault_address)
	 && !rw_page_fault)
	{
	  printf("Code");
	  ...
	  send_to_controller(CODE_PAGE_FAULT,0);
				...
				/* in BSS ? */
      else if((dword_t) & _edata <= fault_address 
	 && ((dword_t) & _end > fault_address))
	{
	  printf("BSS");
	  fp.fpage = l4_fpage(fault_address, L4_LOG2_PAGESIZE, 
      ...
      if((!handled_by_client) && snd_desc == L4_IPC_SHORT_FPAGE)
	{
	  ...
	  if(fp.fpage.iofp.f == 0xf)
	    ...
	    w0 = fp.fpage.fp.page << PAGE_SHIFT | fp.fpage.fp.write;

	  /* get this page from my pager */
	  res = l4_i386_ipc_call(my_pager,		  // destination
	  ...
	  if(snd_desc == L4_IPC_SHORT_FPAGE)
	    if(fp.fpage.iofp.f == 0xf)
	      ...
	      printf("\n mem fpage %s (%s), base : %08x, size : %d, "
		     ...
		     fp.fpage.fp.page << PAGE_SHIFT,
		     fp.fpage.fp.size,
		     fp.snd_base
		     );
	  else
	    printf("\n word0 : %08x, word 1 : %08x]\n",
		   fp.snd_base,
		   fp.fpage.fpage);
	}
      else
...
void init_get_next_stack(void)
{
  ...
  stack_pointer = (reinterpret_cast<vm_offset_t>(stack2) +
...
int generic_main(void)
{
  ...
  printf("Starting test task, idt : %08x-%08x, ESP : %08x\n",
