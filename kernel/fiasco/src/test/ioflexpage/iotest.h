/* $Id$
 *
 * include file for writing l4 testcases 
 *
 * see iotest.cc for comments
 *
 */
 ...
 */
void receive(controller_message_t *w0, dword_t * w1);


/* receive until messages != ERROR_INFO
 * code must be a nonnull pointer
 */
void receive_no_info(controller_message_t *w0, dword_t *w1);

/* check for condition cond, if false print some diagnostics and 
 * TEST FAILED TEST FAILED TEST FAILED TEST FAILED
 */
void controller_check(controller_message_t code, dword_t w1, bool cond);
...
extern bool handle_page_fault(dword_t fault_address, dword_t fault_eip,
