...
bool handle_interrupt(void)
{
  ...
  if(((* reinterpret_cast<dword_t *>(regs.eip)) & 
      (0x0000ffff |		// op code
    ...
    {
      dword_t address = (* reinterpret_cast<dword_t *>(regs.eip +3));
      printf("Acessing %08x, skip instruction\n", address);
      regs.eip += 7;
      regs.eax = 0;
