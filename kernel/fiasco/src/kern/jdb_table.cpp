...
IMPLEMENT
bool
Jdb_table::edit_entry(unsigned, unsigned, unsigned, unsigned)
{ return false; }
...
IMPLEMENT
unsigned 
Jdb_table::key_pressed(int, unsigned &, unsigned &)
{ return Nothing; }
...
PRIVATE 
unsigned
Jdb_table::vis_cols(unsigned first_col, unsigned *w)
{
  unsigned c = 0;
  *w = 0;
...
PRIVATE
unsigned
Jdb_table::col_ofs(unsigned first_col, unsigned col)
{
  unsigned c = 0;
  unsigned w = 0;
  for (;c<col; ++c)
...
PUBLIC
bool
Jdb_table::show(unsigned crow,unsigned ccol)
{
  unsigned tmp;
  unsigned absr = 0, absc = 0;
  unsigned old_absc = 1, old_absr = 1;
  unsigned dcols, drows;        // drawn columns and rows
  drows = height();
  dcols = vis_cols(absc, &tmp);
  if (drows>rows()) drows = rows();

  unsigned max_absr = 0, max_absc = 0;
  if (rows() > drows) max_absr = rows() - drows;
//  if (cols() > dcols) max_absc = cols() - dcols;

  unsigned min_row, min_col;
...
PUBLIC
void
Jdb_table::draw_table(unsigned row, unsigned col, 
  ...
  for (unsigned y = 0; y < lines; ++y)
      ...
      if (w < width())
	{
	  if (width() == Jdb_screen::width())
	    ...
	    printf("\033[%dX", width()-w); // some consoles don't support this
	}
      putchar('\n');
    }
}
