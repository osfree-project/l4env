...
Thread::do_send_long (Thread *partner, Sys_ipc_frame *i_regs)
{
  ...
  if (EXPECT_TRUE ((error = setjmp(pf_recovery)) == 0) )
	...
	rcv_descr = (message_header*) _target_desc.msg();
	 ...
	 return ipc_finish (partner, Ipc_err::Remsgcut, dont_switch);
      ...
      L4_fpage rcv_fpage (rcv_descr
                          ? partner->space()->peek_user (&rcv_descr->fp)
                          : _target_desc.fpage());
      ...
      if (!rcv_descr)
	{
      ...
      L4_msgdope rcv_sizedope (partner->space()->
      ...
      if (min > Sys_ipc_frame::num_reg_words())
	{
	  ...
	  space()->copy_user_to_user (partner->space(),
      ...
      while (snd_strings && rcv_strings)
	{
	  ...
	  Address    from_size = space()->peek_user (&snd_strdope->snd_size);
	  Unsigned8 *to        = partner->space()->
				    peek_user (&rcv_strdope->rcv_str);
	  Address    to_size   = partner->space()->
	  ...
	  if (min > 0)
	    space()->copy_user_to_user (partner->space(), to, from, min);

	  // indicate size of received data
	  // XXX also overwrite rcv_strdope->snd_str?
	  partner->space()->poke_user (&rcv_strdope->snd_size, min);
