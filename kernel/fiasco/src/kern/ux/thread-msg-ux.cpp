...
IMPLEMENT inline
Mword
Thread::update_ipc_window (Address, Address)
{
  return 0;	// We neither have an IPC window, nor do we copy mappings.
}
...
Thread::do_send_long (Thread *partner, Sys_ipc_frame *i_regs)
{
  ...
  if (min > Sys_ipc_frame::num_reg_words()) {
    ...
    copy_user_to_user (partner, (Address) (rcv_descr->words +
                                           Sys_ipc_frame::num_reg_words()),
                                (Address) (snd_descr->words +
                                           Sys_ipc_frame::num_reg_words()),
  ...
  while (snd_strings && rcv_strings) {
    ...
    Address from      = peek_user (&snd_strdope->snd_str);
    Address from_size = peek_user (&snd_strdope->snd_size);
    Address to        = partner->peek_user (&rcv_strdope->rcv_str);
