...
Jdb_misc_debug::action(int cmd, void *&args, char const *&fmt, int &)
{
  switch (cmd)
      ...
      else
	{
	  ...
	  if (Cpu::lbr_type() == Cpu::Lbr_pentium_4 || 
	      Cpu::lbr_type() == Cpu::Lbr_pentium_4_ext)
	    {
	      Unsigned64 msr;
	  ...
	  else if (Cpu::lbr_type() == Cpu::Lbr_pentium_6)
	    {
	      Unsigned64 msr;
...
{
  Mem_space *s = Space_index(task).lookup()->mem_space();
  Address addr, size;
  ...
  printf("\nLDT of space %x at "L4_PTR_FMT"-"L4_PTR_FMT"\n", task, addr, addr+size-1);
