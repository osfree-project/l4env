...
PUBLIC
Jdb_module::Action_code Jdb_attach_irq::action( int cmd, void *&args, char const *&fmt )
{
  if(cmd!=0)
    return NOTHING;
	...
	case 'l': // list
	  {
	    ...
	    while((r=Irq_alloc::lookup(i++)))
	      {
		printf("\nIRQ %02x: %p (%s)", i-1, r->owner(), ((Mword)r->owner()==0)?
		       "unassigned":
		       (((Smword)r->owner()==-1)?"JDB":"user") );
	      }
	    putchar('\n');
	  }
	  return NOTHING;
...
Jdb_module::Cmd const *const Jdb_attach_irq::cmds() const
{
  static Cmd cs[] =
    { Cmd( 0, "R", "irq", " cmd: %c", 
	   "R{l|a}xxx\tlist IRQ threads, attach Jdb to IRQ", (void*)&subcmd )
    };
