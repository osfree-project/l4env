...
{
  // this == current_mem_space() does not apply for SMAS
  return user_space ? peek_user (addr) : *addr;
}

IMPLEMENTATION[arm,{ia32,amd64}-!smas]:
...
IMPLEMENT inline NEEDS [<cassert>, "cpu.h", Mem_space::current_pdir]
template < typename T >
void
Mem_space::copy_from_user (T *kdst, T const *usrc, size_t n)
{
...
Mem_space::peek_user (T const *addr)
{
  ...
  switch (sizeof (T))
    {
      case 1:
        asm ("movb %%fs:%1, %b0" : "=r" (ret) : "m" (*addr));
        break;
      case 2:
        asm ("movw %%fs:%1, %w0" : "=r" (ret) : "m" (*addr));
        break;
      case 4:
        asm ("movl %%fs:%1,  %0" : "=r" (ret) : "m" (*addr));
        break;
      default:
...
Mem_space::poke_user (T *addr, T value)
{
  ...
  switch (sizeof (T))
    {
      case 1:
        asm ("movb %b0, %%fs:%1" : : "Nr" (value), "m" (*addr) : "memory");
        break;
      case 2:
        asm ("movw %w0, %%fs:%1" : : "Nr" (value), "m" (*addr) : "memory");
        break;
      case 4:
        asm ("movl  %0, %%fs:%1" : : "Nr" (value), "m" (*addr) : "memory");
        break;
      default:
