...
environment to be setup up. This environment contains IDL compilers,
...
\todo{Mention that we do not need the whole environment for our
tutorial. The different things (Linux kernels, OSKit, tools) must be
mentioned in the cited tutorial separately, and we have to say here
which sections in this tutorial require which things. In the cited
tutorial, also mention that if you are at the Operating Systems Group
at TU~Dresden (TUD) with access to the file server
\name{kurt.inf.tu-dresden.de}, most of these files are already
installed at /home/drops/. Mention which files.
}
...
\begin{descriptiontable}{oskit10}
  l4/           & the L4 directory, later referenced as §(L4DIR). \\
  oskit/        & the ``old'' FLUX OSKit~0.6, a libc. \\
  oskit10/      & the ``new'' FLUX OSKit~1.0, a collection of libcs.
                  \\
  linux22/      & L4Linux 2.2 \\
...
\begin{descriptiontable}{l4/include}
  l4/kernel     & sources of L4 micro kernel implementations, among
                  them FIASCO. \\
...
«§(PKDIR)». Switch to the directory and use the template provided by
...
files will go into. You will find the compiled binary in this
...
into «idl/» and create the file «hiworld.idl»:
...
the «include/» directory and create the new file «hiworld.h»:
...
Client libraries are typically build in the «lib/» directory of a
...
server, and consequently place it into the «examples/» directory tree.
...
(probably nothing actually gets actually built, as everything should
...
\todo{This section should go into the building tutorial mentioned in
...
installed into this directory. Thus at \emph{erwin.inf.tu-dresden.de}:

|kurt:/home/yourname> mkdir -p boot/bin|\\
|kurt:/home/yourname> ln -s /tftpboot/yourname boot|

(The latter may require administrator rights, ask the administrator in
this case.) At your \emph{workstation}, where kurt:oshome is mounted
(we assume /home to be the mountpoint):

|~/src/l4> ln -s /home/YOURNAME/boot/bin|
