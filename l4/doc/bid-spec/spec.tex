...
\author{Jork Löser\\drops@os.inf.tu-dresden.de}
\date{\today}
\maketitle
\thispagestyle{empty}
...
This documentation is \copyright2003 by Technische Universität
...
\begin{tabular}{|l|c|c|c|}\hline
        ARCH    & \underline{x86}              & arm & ia64 \\\hline
        CPU     & \underline{586}, 686, K6, K7 & \underline{sa}, pxa & \\\hline
        L4API   & \multicolumn{3}{c|}{\underline{l4v2}, l4x0, l4x2} \\\hline
\end{tabular}
...
l4env_oskit10 & This is an alias for \name{l4env}. \\
...
sigma0_oskit06 & This is an alias for \name{sigma0}. \\
...
l4linux      & Use this mode to build \llinux{} user-mode applications
               ...
               against standard-libc. \\

l4linux_kern & Use this mode to build \llinux{} kernel-mode modules
...
Tables \ref{tab:mode-incdirs}, \ref{tab:mode-libdirs}, \ref{tab:mode-clibs},
\ref{tab:mode-l4libs} and \ref{tab:mode-crt0ldscript} for what is determined
by each mode. See Section \ref{sec:prog.mk} and Section
\ref{sec:lib.mk} for the meaning of the variables.
...
l4env_freebsd   & -nostdinc -DOSKIT -I\{
                  §(L4DIR)/../oskit10/freebsd/ \{libc/include,
                  3.x/src/include, 3.x/src/sys\} §(L4DIR)/../oskit10
...
loader          & nostdinc -DOSKIT -I\{
                  §(L4DIR)/../oskit10 §(L4DIR)/../oskit10/oskit/c
                  §(DROPS_STDDIR)/include/oskit10
                  §(DROPS_STDDIR)/include/oskit10/oskit/c \}
                &\\\hline

sigma0          & -nostdinc -I\{§(L4DIR)/../oskit/ §(L4DIR)/../oskit/flux/c
...
l4linux_kern    & -nostdinc -I§(L4LINUX22_DIR)/include -D__KERNEL__
                  -DLINUX_ON_L4
                &\\\hline
...
\end{ptabtwo}
\caption{Directories for include file search depending on the mode.}
\label{tab:mode-incdirs}
\end{table}
...
l4env           & -L§(L4DIR)/../oskit10/lib
                  -L§(DROPS_STDDIR)/lib/oskit10 &\\\hline

l4env_freebsd   & -L\{§(L4DIR)/../oskit10/lib 
                  -L§(L4DIR)/lib/c++/§(GCCVERSION)
                  §(DROPS_STDDIR)/lib/oskit10
                  §(DROPS_STDDIR)/lib/c++/§(GCCVERSION)
                  \} &\\\hline

loader          & -L§(L4DIR)/../oskit10/lib
                  -L§(DROPS_STDDIR)/lib/oskit10 &\\\hline

sigma0          & -L§(L4DIR)/../oskit/lib -L§(DROPS_STDDIR)/lib/oskit06
                &\\\hline

oskit10_sigma0  & -L§(L4DIR)/../oskit10/lib
                  -L§(DROPS_STDDIR)/lib/oskit10 &\\\hline

l4linux         & -L§(DROPS_STDDIR)/lib &\\\hline
...
l4env_freebsd   & -nostdlib -loskit10_support_l4env_full -loskit_startup
                  ...
                  -loskit_freebsd_c &\\\hline

loader          & -nostdlib
                  -loskit10_support_l4env -loskit_c
                  -loskit_kern -loskit_clientos -loskit_lmm
                  -loskit10_support_l4env -loskit_c
                  -loskit_kern -loskit10_support_l4env &\\\hline

sigma0          & -nostdlib -lmc -loskit_support -lkern -llmm -lmc
                &\\\hline
...
l4linux         & -ldm_generic -ldm_mem -lnames -ll4util
...
l4env           & -static -ll4env -ll4rm -ldm_generic -ldm_mem -lthread
                  ...
                  -loskit10_support_l4env -ll4env -ll4env_err -ll4rm 
                  -ldm_generic -ldm_mem -lthread -lslab -ll4sys §(GCCLIB)
                &\\\hline

l4env_freebsd   & -static -ll4env_oskit10 -ll4rm -ldm_generic
                  -ldm_mem -lthread -lsemaphore -llogserver -lnames
                  -ll4util -lrmgr -loskit10_support_l4env_full
                  -ll4env_oskit10 -ll4env_err -ll4rm -ldm_generic
                  -ldm_mem -lthread -lslab -ll4sys §(GCCLIB)
                  §(GCCLIB_EH) &\\\hline

loader          & -lloader.s -ll4sys §(GCCLIB) &\\\hline

sigma0          & -static -llogserver -lnames -ll4util -lrmgr -ll4sys §(GCCLIB)
                &\\\hline

oskit10_sigma0  & -static -llogserver -lnames -ll4util -lrmgr -ll4sys §(GCCLIB)
                &\\\hline
...
l4env_freebsd   & crt0_l4env_freebsd.o
                & main_stat.ld
                & \\\hline

loader          & crt0_l4env_dyn.o
                & main_stat.ld
                &\\\hline
...
oskit10_sigma0  & crt0_oskit10_support.o
                & main_stat.ld
                &\\\hline
...
«CRT0_arch_mode», «CRTN_arch_mode» and «LDSCRIPT_x86_sigma0».
...
\vfill\pagebreak
\section{Relocation of Binaries}
\label{sec:reloc}
\index{relocation}
...
\subsection{Relocated binaries and the STATIC file}
...
\subsection{Relocatable binaries}
...
final start-address is not defined yet. These binaries are not
executable yet. After building the binaries in this relocatable form,
the size of all binaries is known. Then, all binaries are relocated
...
\item [reloc::]         \index{relocation!reloc:: make target}
                        ...
                        Section~\ref{sec:reloc-bins}). Used by
...
For full-featured C++ support, you have to use the l4env_freebsd mode. 
...
\todo{Shall we link libprofile.a to the binary if profiling is enabled?}
...
\item [dvi::, ps::] Phony targets. Compile the primary tex document
                    into dvi and ps, respectively.

\item [showdvi::, showps::] Phony targets. Invoke a viewer on the
...
\item [genexp::] Generate the expected output.
...
\begin{parameterlist}{REQUIRED_SERVERS}
    ...
    BASE_SERVERS	& \index{§(BASE_SERVERS)}%
			  ...
			  of user-land executables.  Otherwise it is take as
			  is.
...
\item [Install mk-files, open] Install the *.mk implementations to
