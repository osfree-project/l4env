...
void sent_to(l4_threadid_t dest, l4_umword_t w0)
{
  int res;
  l4_msgdope_t dope;
  l4_threadid_t my_id;
  ...
  if(debugcommunication > 10)
    {
      ...
      printf("%s : send %ld to %s\n",
	     task_to_name(my_id), w0, task_to_name(dest));
    }

  res = l4_ipc_send(dest,
...
void sent_page_to(l4_threadid_t dest, l4_umword_t virt)
{
  int res;
    l4_threadid_t my_id;
  l4_msgdope_t dope;
...
l4_umword_t receive_from(l4_threadid_t src, l4_umword_t expect)
{
  int res;
  l4_msgdope_t dope;
  l4_umword_t _w0, _w1;
  ...
  if(_w0 != expect){
    ...
    printf("%s : receiving unexpected %ld (%s -> %s)\n",
...
void receive_page_from(l4_threadid_t src, l4_umword_t mapaddr)
{
  l4_msgdope_t dope;
  int res;
  l4_umword_t ignore;
  ...
  if(debugcommunication)
    {
      ...
      printf("%s : receive page %08x from %s\n",
	     task_to_name(my_id), fp.fp.page << L4_PAGESHIFT, 
	     task_to_name(src));
    }
}
...
{
  l4_umword_t mappaddr = 0x20001000;
...
void hans_task(void)
{
  l4_umword_t mappaddr = 0x20002000;
...
void axel_task(void)
{
  l4_umword_t mappaddr = 0x20003000;
...
void susi_task(void)
{
  l4_umword_t mappaddr = 0x20004000;
  ...
  printf("susi: accessing %08lx : ", mappaddr);
  l4_umword_t x = * reinterpret_cast<l4_umword_t *>(mappaddr);
  printf("%08lx\n", x);
...
{
  l4_umword_t mappaddr = 0x20005000;
  ...
  printf("annemarie: flush page %08lx\n",
	 mappaddr);
...
int main(void)
{
  ...
  l4_umword_t friedrich_stack_base, hans_stack_base, annemarie_stack_base;
  l4_umword_t axel_stack_base, susi_stack_base;
  ...
  printf("Starting task friedrich (%d), idt : %08lx-%08lx, ESP : %08lx\n",
  ...
  friedrich_id = 
    l4_task_new(friedrich_id,			 /* dest task */
		...
		(l4_umword_t)friedrich_task,	 /* EIP */
  ...
  printf("Starting task hans (%d), idt : %08lx-%08lx, ESP : %08lx\n",
	 hans_id.id.task,
	 reinterpret_cast<l4_umword_t>(idt_table), 
	 reinterpret_cast<l4_umword_t>(idt_table+idt_gates), 
	 hans_stack_base);
  ...
  printf("Starting task axel (%d), idt : %08lx-%08lx, ESP : %08lx\n",
	 axel_id.id.task,
	 reinterpret_cast<l4_umword_t>(idt_table), 
	 reinterpret_cast<l4_umword_t>(idt_table+idt_gates), 
	 axel_stack_base);
  ...
  printf("Starting task susi (%d), idt : %08lx-%08lx, ESP : %08lx\n",
	 susi_id.id.task,
	 reinterpret_cast<l4_umword_t>(idt_table), 
	 reinterpret_cast<l4_umword_t>(idt_table+idt_gates), 
	 susi_stack_base);
  ...
  printf("Starting task annemarie (%d), idt : %08lx-%08lx, ESP : %08lx\n",
  ...
  annemarie_id = 
    l4_task_new(annemarie_id,			 /* dest task */
		...
		(l4_umword_t)annemarie_task,	 /* EIP */
		pager_thread_id );		 /* pager */
  sit_and_wait(SHORT_WAIT);
