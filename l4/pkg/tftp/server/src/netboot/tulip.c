...
#define STATUS(fmt...) \
  do { \
      ...
      long diff, now; \
      asm volatile ("divl %%esi\n\t" : "=a"(now) : "A"(ns), "S" (1000000)); \
      diff=now-status; \
      printf("... "); \
      if (diff<40000) printf("%ld ms ",diff); \
      printf(fmt); \
      status=now; \
  } while (0)
#endif
...
#undef TULIP_FORCE_BNC

#define TX_TIME_OUT       2*TICKS_PER_SEC

static int tulip_id = 0;

static unsigned save_csr3 = 0;
static unsigned save_csr4 = 0;
static unsigned save_csr7 = 0;
...
static void tulip_reset(struct nic *nic)
{
  ...
  if (vendor == PCI_VENDOR_ID_MACRONIX && dev_id == PCI_DEVICE_ID_MACRONIX_MX987x5) {
  ...
  } else if (vendor == PCI_VENDOR_ID_LINKSYS && dev_id == PCI_DEVICE_ID_LINKSYS_LC82C115) {
  ...
  for (i=0; (i<20) && ((inl(ioaddr + CSR5) & 0x10) == 0); i++)
    l4_sleep(100);
...

/*********************************************************************/
/* eth_enable - (Re-)Enable the interface                                 */
/*********************************************************************/
static void tulip_enable(struct nic *nic)
{
  whereami("tulip_enable\n");
  
  /* Init ring */
  tulip_init_ring(nic);

  /* save olf send/receive decriptors */
  save_csr3 = inl(ioaddr + CSR3);
  save_csr4 = inl(ioaddr + CSR4);
  save_csr7 = inl(ioaddr + CSR7);

  /* Point to send/receive descriptors */
  outl((unsigned long)&rxd[0], ioaddr + CSR3);
  outl((unsigned long)&txd   , ioaddr + CSR4);

  /* Start the chip's Rx processes. */
  outl(inl(ioaddr + CSR6) | 0x00000002, ioaddr + CSR6);
                          ...
                          struct pci_device *pci)
{
  ...
  printf("Tulip %02x:%02x:%02x:%02x:%02x:%02x rev %02x at ioaddr 0x%lx\n",
    nic->node_addr[0] & 0xff, nic->node_addr[1] & 0xff, 
    nic->node_addr[2] & 0xff, nic->node_addr[3] & 0xff,
    nic->node_addr[4] & 0xff, nic->node_addr[5] & 0xff,
    chip_rev, ioaddr);
