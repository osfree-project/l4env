...
#ifdef INCLUDE_3C509
/*
 * Normally the isa_drivers and isa_drivers_end symbols are generated
 * by the linker script. In our case this is a bit difficult as
 * our build system can only have one linker script (AFAIK) and we'd need an
 * additional snippet to get this working. Copying and modifying the linker
 * script is not an option.
 * Big downside: adding another ISA driver won't be that easy but is
...
asm(".section .data		\t\n"
    ...
    ".globl isa_drivers		\t\n"
    "isa_drivers:		\t\n");
struct isa_driver t509_driver __attribute__((unused)) = {
#else
