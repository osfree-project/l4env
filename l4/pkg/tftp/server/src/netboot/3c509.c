...
static unsigned char	eth_vendor; /* , eth_flags, eth_laar; -> unused */
static unsigned short	eth_nic_base; /* , eth_asic_base; -> unused */
static char		bnc=0, utp=0; /* for 3C509 */
static char		first_probe=1; /* probe is used in enable */
...
#define init_printf(format, args...) { if (first_probe) printf(format, ## args); } while (0)
...
{
	/* Anyone knows a better way for enable? */
	t509_probe(nic, 0, 0);
}
...
#endif
{
	...
	for (failcount=0; failcount<1; failcount++) {
...
#ifdef	INCLUDE_3C529
	if (mcafound) {
		init_printf("%s board found on MCA at 0x%x IRQ %d -",
				mcafound->name, eth_nic_base, mca_irq);
	} else {
#endif
		if(eth_nic_base >= EP_EISA_START) {
			init_printf("3C5x9 board on EISA at 0x%x - ",eth_nic_base);
		} else {
			init_printf("3C5x9 board on ISA at 0x%x - ",eth_nic_base);
		}
#ifdef	INCLUDE_3C529
	}
#endif
	...
	switch(j) {
		case 0:
			if(i & IS_UTP) {
				...
				init_printf("10baseT not present\n");
				eth_vendor=VENDOR_NONE;
				goto no3c509;
			}
		...
		case 1:
			if(i & IS_AUI)
				...
				init_printf("10base5 not present\n");
				eth_vendor=VENDOR_NONE;
				goto no3c509;
			}
		...
		case 3:
			if(i & IS_BNC) {
				...
				init_printf("10base2 not present\n");
				eth_vendor=VENDOR_NONE;
				goto no3c509;
			}
	...
	*/
	...
	init_printf("Ethernet address: ");
	for(i=0; i<5; i++) {
		init_printf("%x:",nic->node_addr[i] & 0xff);
	}
	init_printf("%x\n",nic->node_addr[i] & 0xff);

	t509_reset(nic);
	nic->reset = t509_reset;
	nic->poll = t509_poll;
	nic->transmit = t509_transmit;
	nic->enable = t509_enable;
