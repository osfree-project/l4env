...
static struct pcnet32_init_block {
    word mode;
    word tlen_rlen;
    byte phys_addr[6];
    word reserved;
...
static void
pcnet32_reset(struct nic *nic)
{
};


static void
pcnet32_disable(struct nic *nic)
{
  /* reset */
...
pcnet32_init(struct nic *nic)
{
    ...
    printf("AMD-PCNet32 base 0x%lx, addr ", iobase);

    pcnet32_getaddress(nic->node_addr);
    for (i=0; i<ETHER_ADDR_SIZE; i++) 
      {
	printf("%x", (unsigned char) (nic->node_addr[i]));
...
static void
pcnet32_transmit(struct nic *nic, const char *d, 
                 unsigned int t, unsigned int s, const char *p)         
{
    ...
    for (i = 0; i < ETHER_ADDR_SIZE; i++)
      {
        tx_buff[i] = d[i];
        tx_buff[ETHER_ADDR_SIZE + i] = nic->node_addr[i];
      };
    /* copy packet type to buffer */
    tx_buff[2*ETHER_ADDR_SIZE]     = (t >> 8) & 0xFF;
    tx_buff[2*ETHER_ADDR_SIZE + 1] =  t       & 0xFF;
    /* copy the whole packet, too */
    for (i = 0; i < s; i++)
        tx_buff[2*ETHER_ADDR_SIZE + 2 + i] = p[i];

    s += 2*ETHER_ADDR_SIZE + 2;
...
pcnet32_poll(struct nic *nic)
{
    ...
    for (i = 0; i < s; i++)
        nic->packet[i] = p[i];
    nic->packetlen = s;           /* available to caller */
    return 1;
};


struct nic*
pcnet32_probe(struct nic *nic, unsigned short *probe_addrs, struct pci_device *pci);
