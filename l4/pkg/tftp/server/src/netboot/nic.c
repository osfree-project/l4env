...
int ip_transmit(int len, const void *buf)
{
	...
	if (destip == IP_BROADCAST) {
		...
		if (arpentry == MAX_ARP) {
			printf("%ld.%ld.%ld.%ld is not in my arp table!\n", 
			    (destip & 0xff000000) >> 24,
			    (destip & 0xff000000) >> 16,
			    (destip & 0xff000000) >> 8,
			    (destip & 0xff000000));
			return(0);
		}
...
void print_network_configuration (void)
{
	...
	if (! eth_probe ())
		...
		printf ("  Address: %d.%d.%d.%d\tNetmask: %ld.%ld.%ld.%ld\n", 
		    (arptable[ARP_CLIENT].ipaddr.s_addr & 0x000000ff),
		    (arptable[ARP_CLIENT].ipaddr.s_addr & 0x0000ff00) >> 8,
		    (arptable[ARP_CLIENT].ipaddr.s_addr & 0x00ff0000) >> 16,
		    (arptable[ARP_CLIENT].ipaddr.s_addr & 0xff000000) >> 24,
		    (netmask & 0x000000ff),
		    (netmask & 0x0000ff00) >> 8,
		    (netmask & 0x00ff0000) >> 16,
		    (netmask & 0xff000000) >> 24);
		printf ("   Server: %d.%d.%d.%d\tGateway: %d.%d.%d.%d\n", 
		    (arptable[ARP_SERVER].ipaddr.s_addr & 0x000000ff),
		    (arptable[ARP_SERVER].ipaddr.s_addr & 0x0000ff00) >> 8,
		    (arptable[ARP_SERVER].ipaddr.s_addr & 0x00ff0000) >> 16,
		    (arptable[ARP_SERVER].ipaddr.s_addr & 0xff000000) >> 24,
		    (arptable[ARP_GATEWAY].ipaddr.s_addr & 0x000000ff),
		    (arptable[ARP_GATEWAY].ipaddr.s_addr & 0x0000ff00) >> 8,
		    (arptable[ARP_GATEWAY].ipaddr.s_addr & 0x00ff0000) >> 16,
		    (arptable[ARP_GATEWAY].ipaddr.s_addr & 0xff000000) >> 24);
	}
