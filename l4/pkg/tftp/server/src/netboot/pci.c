...
#if CONFIG_VIA_RHINE==1
extern struct pci_driver rhine_driver;
#endif/* CONFIG_VIA_RHINE */

#if CONFIG_W89C840==1
extern struct pci_driver w89c840_driver;
#endif /* CONFIG_W89C840 */

#if CONFIG_SUNDANCE==1
...
void
find_pci (int type, struct pci_device *dev)
{
  return io_support_scan_pci_bus(type, dev);
}

void show_pci_drivers (int *col, int *comma);

void
show_pci_drivers (int *col, int *comma)
{
  int i;

  for(i=0; pci_drivers[i] != 0; i++)
    {
      *col += strlen(pci_drivers[i]->name);
      if (*comma)
	*col += 2;
      if (*col > 70)
	{
