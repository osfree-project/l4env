...
print_network_configuration (void)
{
  ...
  if (! eth_probe ())
    ...
    {
      char me[24], my_mask[24], server[24], gw[24];
	      ...
	      unsigned int destsock, int len, const void *buf)
{
  struct iphdr *ip;
  struct udphdr *udp;
  static struct arprequest arpreq;
  int arpentry, i;
  int retry;
  ...
  if (destip == IP_BROADCAST)
	...
	if (arptable[arpentry].ipaddr.s_addr == destip)
      ...
      if (arpentry == MAX_ARP)
	{
	  grub_printf ("%lx is not in my arp table!\n", destip);
	  return 0;
	}
...
rarp (void)
{
  ...
  static struct arprequest rarpreq;
...
await_reply (int type, int ival, void *ptr, int timeout)
{
  ...
  struct iphdr *ip;
  struct udphdr *udp;
  static struct arprequest *arpreply;
