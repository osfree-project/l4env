...
/*****************************************************************************/
/**
 * \file   l4env/lib/src/startup_oskit10.c  
 ...
 *
 * L4Env task startup code, full OSKit 1.0 support.
...
extern unsigned long base_stack_start;
extern unsigned long base_stack_end;

/* RMGR trampoline page */
extern long _tramppage;

/* application main (XXX we don't need parameters here) */
...
__setup_fixed(void)
{
  ...
  /* reserve area where our modules (e.g. files for memfs) live */
...
/*****************************************************************************/ 
void 
l4env_startup(l4_addr_t boot_info);

void
l4env_startup(l4_addr_t _boot_info)
{
  int ret,i;
  l4_umword_t dummy;
  ...
  boot_info = (struct multiboot_info *)_boot_info;
  ...
  ret = l4thread_setup(l4_myself(),(l4_addr_t)&base_stack_start,
		       (l4_addr_t)&base_stack_end);
  if (ret < 0)
    {
  ...
  /* init semaphore lib */
  ret = l4semaphore_init();
  if (ret < 0)
    {
      printf("Startup: semaphore lib initialization failed\n");
      enter_kdebug("PANIC");
    }

  /* start thread */
  ret = l4thread_create((l4thread_fn_t)multiboot_main,(void*)boot_info,
