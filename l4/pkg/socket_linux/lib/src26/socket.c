...
static struct net_proto_family const *net_families[NPROTO];
...
{
//	LOG("Check me - sock_create_kern");
	return sock_create(family, type, protocol, res);
}

int sock_create_lite(int family, int type, int protocol, struct socket **res)
{
	LOG("Check me - sock_create_lite");
	return sock_create(family, type, protocol, res);
}
...
int sock_sendmsg(struct socket *sock, struct msghdr *msg, size_t size)
{
  ...
  kiocb.private = &siocb;
  ret = __sock_sendmsg(&kiocb, sock, msg, size);
  if (-EIOCBQUEUED == ret)
...
int recvmsg(int sd, struct msghdr *msg, int flags)
{
	...
	err = sock->ops->recvmsg(NULL, sock, &msg_sys, total_len, flags);
	if (err < 0)
		goto out_freeiov;
...
int socket_ioctl(int sd, unsigned int cmd, void *arg)
{
	...
	err = sock->ops->ioctl(sock, cmd, (unsigned long)arg);
        ...
        // if the IO is unknown, try to ask the NIC directly.
	if (err == -ENOIOCTLCMD)
  	  err = dev_ioctl(cmd, arg);
