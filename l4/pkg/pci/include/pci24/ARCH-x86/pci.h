...
#else
#include <l4/pci24/list.h>
#include <l4/pci24/ioport.h>
#endif
 ...
 */
struct pci_dev {
	struct pci_list_head global_list;	/* node in list of all PCI devices */
	struct pci_list_head bus_list;	/* node in per-bus list */
	 ...
	 */
	unsigned int	irq;
	struct pci_resource resource[DEVICE_COUNT_RESOURCE]; /* I/O and memory regions + expansion ROMs */
	struct pci_resource dma_resource[DEVICE_COUNT_DMA];
	struct pci_resource irq_resource[DEVICE_COUNT_IRQ];
...
struct pci_bus {
	struct pci_list_head node;		/* node in list of buses */
	struct pci_bus	*parent;	/* parent bus this bridge is on */
	struct pci_list_head children;	/* list of child buses */
	struct pci_list_head devices;	/* list of devices on this bus */
	struct pci_dev	*self;		/* bridge device as seen by parent */
	struct pci_resource	*resource[4];	/* address space routed to this bus */
...
extern struct pci_list_head pci_root_buses;	/* list of all known PCI buses */
extern struct pci_list_head pci_devices;	/* list of all devices */
...
void pcibios_align_resource(void *, struct pci_resource *, unsigned long);
void pcibios_update_resource(struct pci_dev *, struct pci_resource *,
			     struct pci_resource *, int);
...
void pci_init(void);
int pci_bus_exists(const struct pci_list_head *list, int nr);
...
struct pci_resource *pci_find_parent_resource(const struct pci_dev *dev, struct pci_resource *res);
...
void pdev_sort_resources(struct pci_dev *, struct pci_resource_list *, u32);
