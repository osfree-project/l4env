...
{
        struct pci_resource *res = &dev->resource[resource];
	struct pci_resource *root = pci_find_parent_resource(dev, res);
	int err;
	...
	int resno)
{
	...
	type_mask |= PCI_IORESOURCE_IO | PCI_IORESOURCE_MEM;
	for (i = 0 ; i < 4; i++) {
		...
		if ((r->flags & PCI_IORESOURCE_PREFETCH) && !(res->flags & PCI_IORESOURCE_PREFETCH))
			continue;
...
pci_assign_resource(struct pci_dev *dev, int i)
{
	...
	min = (res->flags & PCI_IORESOURCE_IO) ? PCIBIOS_MIN_IO : PCIBIOS_MIN_MEM;

	/* First, try exact prefetching match.. */
	if (pci_assign_bus_resource(bus, dev, res, size, min, PCI_IORESOURCE_PREFETCH, i) < 0) {
		/*
		 * That failed.
		 *
		 ...
		 */
		if (!(res->flags & PCI_IORESOURCE_PREFETCH) || pci_assign_bus_resource(bus, dev, res, size, min, 0, i) < 0) {
...
pdev_sort_resources(struct pci_dev *dev,
		    struct pci_resource_list *head, u32 type_mask)
{
	int i;

	for (i = 0; i < PCI_NUM_RESOURCES; i++) {
		struct pci_resource *r;
		struct pci_resource_list *list, *tmp;
		...
		for (list = head; ; list = list->next) {
			...
			struct pci_resource_list *ln = list->next;
...
pdev_enable_device(struct pci_dev *dev)
{
	...
	for (i = 0; i < PCI_NUM_RESOURCES; i++) {
		...
		if (res->flags & PCI_IORESOURCE_IO)
			cmd |= PCI_COMMAND_IO;
		else if (res->flags & PCI_IORESOURCE_MEM)
