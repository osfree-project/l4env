...
PCI_LIST_HEAD(pci_root_buses);
PCI_LIST_HEAD(pci_devices);
...
{
	struct pci_list_head *n = from ? from->global_list.next : pci_devices.next;
...
/**
 * pci_find_parent_resource - return pci_resource region of parent bus of given region
 * @dev: PCI device structure contains pci_resources to be searched
 * @res: child pci_resource record for which parent is sought
 *
 ...
 */
struct pci_resource *
pci_find_parent_resource(const struct pci_dev *dev, struct pci_resource *res)
{
	const struct pci_bus *bus = dev->bus;
	int i;
	struct pci_resource *best = NULL;

	for(i=0; i<4; i++) {
		struct pci_resource *r = bus->resource[i];
		if (!r)
			continue;
		if (res->start && !(res->start >= r->start && res->end <= r->end))
			continue;	/* Not contained */
		if ((res->flags ^ r->flags) & (PCI_IORESOURCE_IO | PCI_IORESOURCE_MEM))
			continue;	/* Wrong type */
		if (!((res->flags ^ r->flags) & PCI_IORESOURCE_PREFETCH))
			return r;	/* Exact match */
		if ((res->flags & PCI_IORESOURCE_PREFETCH) && !(r->flags & PCI_IORESOURCE_PREFETCH))
...
void pci_release_regions(struct pci_dev *pdev)
{
	...
	for (i = 0; i < 6; i++) {
		...
		if (pci_resource_flags(pdev, i) & PCI_IORESOURCE_IO)
			release_region(pci_resource_start(pdev, i),
				       pci_resource_len(pdev, i));

		else if (pci_resource_flags(pdev, i) & PCI_IORESOURCE_MEM)
...
err_out:
	printk (KERN_WARNING "PCI: Unable to reserve %s region #%d:%lx@%lx for device %s\n",
		pci_resource_flags(pdev, i) & PCI_IORESOURCE_IO ? "I/O" : "mem",
		i + 1, /* PCI BAR # */
...
static PCI_LIST_HEAD(pci_drivers);
...
pci_dev_driver(const struct pci_dev *dev)
{
	if (dev->driver)
		...
		for(i=0; i<=PCI_ROM_RESOURCE; i++)
			if (dev->resource[i].flags & PCI_IORESOURCE_BUSY)
...
static inline unsigned int pci_calc_resource_flags(unsigned int flags)
{
	if (flags & PCI_BASE_ADDRESS_SPACE_IO)
		return PCI_IORESOURCE_IO;

	if (flags & PCI_BASE_ADDRESS_MEM_PREFETCH)
		return PCI_IORESOURCE_MEM | PCI_IORESOURCE_PREFETCH;

	return PCI_IORESOURCE_MEM;
}
	...
	if (rom) {
		...
		if (sz && sz != 0xffffffff) {
			res->flags = (l & PCI_ROM_ADDRESS_ENABLE) |
			  PCI_IORESOURCE_MEM | PCI_IORESOURCE_PREFETCH | PCI_IORESOURCE_READONLY | PCI_IORESOURCE_CACHEABLE;
...
void __devinit  pci_read_bridge_bases(struct pci_bus *child)
{
	...
	if (base && base <= limit) {
		res->flags = (io_base_lo & PCI_IO_RANGE_TYPE_MASK) | PCI_IORESOURCE_IO;
		res->start = base;
		res->end = limit + 0xfff;
		res->name = child->name;
	} else {
	...
	if (base && base <= limit) {
		res->flags = (mem_base_lo & PCI_MEMORY_RANGE_TYPE_MASK) | PCI_IORESOURCE_MEM;
		res->start = base;
	...
	if (base && base <= limit) {
		res->flags = (mem_base_lo & PCI_MEMORY_RANGE_TYPE_MASK) | PCI_IORESOURCE_MEM | PCI_IORESOURCE_PREFETCH;
		res->start = base;
		res->end = limit + 0xfffff;
		res->name = child->name;
	} else {
...
static struct pci_bus * __devinit  pci_alloc_bus(void)
{
	...
	if (b) {
		memset(b, 0, sizeof(*b));
		INIT_PCI_LIST_HEAD(&b->children);
		INIT_PCI_LIST_HEAD(&b->devices);
	}
	return b;
}
...
int __devinit  pci_bus_exists(const struct pci_list_head *list, int nr)
{
	const struct pci_list_head *l;
...
struct pci_pool {	/* the pool */
	struct pci_list_head	page_list;
	pci_spinlock_t		lock;
	...
	size_t size, size_t align, size_t allocation, int flags)
{
	...
	retval->dev = pdev;
	INIT_PCI_LIST_HEAD (&retval->page_list);
