...
void
pcibios_update_resource(struct pci_dev *dev, struct pci_resource *root,
			struct pci_resource *res, int resource)
{
	u32 new, check;
	int reg;
 ...
 */
void
pcibios_align_resource(void *data, struct pci_resource *res, unsigned long size)
{
...
static void __init pcibios_allocate_bus_resources(struct pci_list_head *bus_list)
{
	struct pci_list_head *ln;
	struct pci_bus *bus;
	struct pci_dev *dev;
	int idx;
	struct pci_resource *r, *pr;
	...
	pci_for_each_dev(dev) {
		...
		for(idx=0; idx<6; idx++) {
			...
			if ((class == PCI_CLASS_STORAGE_IDE && idx < 4) ||
			    (class == PCI_CLASS_DISPLAY_VGA && (r->flags & PCI_IORESOURCE_IO)))
				continue;
