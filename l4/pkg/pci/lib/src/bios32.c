...
unsigned long bios32_indirect = 0;
...
__initfunc(static int check_pcibios(void))
{
	...
	if ((pcibios_entry = bios32_service(PCI_SERVICE))) {
		pci_indirect = pcibios_entry + PAGE_OFFSET;
...
#ifdef __PIC__
	__asm__("pushl %%ebx; movl %%ecx, %%ebx\n\t"
		"push %%cs; call *(%%esi)\n\t"
		"jc 1f\n\t"
		"xor %%ah, %%ah\n\t"
		"1:\n\t"
...
#ifdef __PIC__
	__asm__("pushl %%ebx; movl %%edx, %%ebx\n\t"
	    	"push %%cs; call *(%%esi)\n\t"
		"jc 1f\n\t"
		"xor %%ah, %%ah\n\t"
		"1:\n\t"
...
__initfunc(static struct pci_access *pci_find_bios(void))
{
	...
	for (check = (union bios32 *) __va(0xe0000);
		...
		if (check->fields.entry >= 0x100000) {
			...
			bios32_indirect = bios32_entry + PAGE_OFFSET;
			if (check_pcibios())
...
__initfunc(static void pcibios_sort(void))
{
