...
void vtpmemu_event_loop(void *data) {
  ...
  struct slocal * slocal = (struct slocal *)data; 
  l4_taskid_t *waitfor = &(slocal->shutdown_on_exit);
  ...
  while(1) {
    ...
    {
      LOG("Got exit event of task "l4util_idfmt". Try to save TPM state ...", l4util_idstr(tid));
      tpm_emulator_shutdown();
      res = l4events_unregister(L4EVENTS_EXIT_CHANNEL);
      if (res != 0)
        l4env_perror("unregistration of events service failed", res);

      res = names_unregister_thread(slocal->vtpm_name, slocal->names_unregister);
      if (!res)
        l4env_perror("unregistration of names service failed", res);
