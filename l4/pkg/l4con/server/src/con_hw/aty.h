...
aty_st_le32(int regindex, unsigned val)
{
  ...
  *((volatile unsigned *)(ati_regbase + regindex)) = val;
}
...
aty_ld_le32(int regindex)
{
  ...
  return *((volatile unsigned *)(ati_regbase + regindex));
}

static inline int
ati_is_dbl_scan(void)
{
  return aty_ld_le32(CRTC_GEN_CNTL) & CRTC_DBL_SCAN_EN;
}

static inline int
ati_is_interlace(void)
{
  return aty_ld_le32(CRTC_GEN_CNTL) & CRTC_INTERLACE_EN;
}
...
ati_wait_vsync(void)
{
  ...
  for (i=0; i<2000000; i++)
    if ((aty_ld_le32(CRTC_INT_CNTL) & CRTC_VBLANK)==0)
      break;
  for (i=0; i<2000000; i++)
    if((aty_ld_le32(CRTC_INT_CNTL) & CRTC_VBLANK))
      break;
}

static inline l4_uint32_t
ati_in_pll(l4_uint32_t addr)
{
  l4_uint32_t in;

  in  = aty_ld_le32(CLOCK_CNTL);
  in &= ~(0x200 | 0xfc00);
  aty_st_le32(CLOCK_CNTL, in | (addr<<10));

  /* read the register value */
  return (aty_ld_le32(CLOCK_CNTL)>>16) & 0xFF;
}

static inline void 
wait_for_fifo(unsigned short entries)
{
  ati_test_for_card_disappeared();
  
  while ((aty_ld_le32(FIFO_STAT) & 0xffff) > ((unsigned)(0x8000 >> entries)));
}
