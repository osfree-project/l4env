...
{
    yuv2rgb_render = NULL;
    if (yuv2rgb_render == NULL && (mm_accel () & MM_ACCEL_X86_MMXEXT)) {
	yuv2rgb_render = yuv2rgb_init_mmxext (bpp, mode);
	if (yuv2rgb_render != NULL)
	    printf ("Using MMXEXT for colorspace transform\n");
    }
    if (yuv2rgb_render == NULL && (mm_accel () & MM_ACCEL_X86_MMX)) {
	yuv2rgb_render = yuv2rgb_init_mmx (bpp, mode);
	if (yuv2rgb_render != NULL)
	...
	yuv2rgb_render = (yuv2rgb_fun)yuv2rgb_c;
    }
}
