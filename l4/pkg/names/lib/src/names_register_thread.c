...
/*!\brief Register a thread with the given name
 * \ingroup clientapi
 *
 * \param  name		0-terminated name to register as.
 * \param  id		thread-ID to register
 *
 * \retval 0		Error. Name is already registerd.
 * \retval !=0		Success.
 *
 * The string name is registered with the given thread_id.
 *
 * \note   The name is not necessarily 0-terminated when being sent to the
 *         server.
 */
int
names_register_thread_weak(const char* name, l4_threadid_t id)
{
  ...
  return names_register_thread_call(ns_id, name, &id, &env);
}
