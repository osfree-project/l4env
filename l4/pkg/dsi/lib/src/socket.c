...
__allocate_socket(void)
{
  ...
  do
    {
      if (l4util_cmpxchg32(&sockets[i].flags,
			   DSI_SOCKET_UNUSED,DSI_SOCKET_USED))
	{
	  /* found */
		  ...
		  l4_uint32_t flags, dsi_socket_t ** socket)
{
  ...
  *socket = NULL;
  ...
  if (l4dm_is_invalid_ds(*data_ds))
    {
      LOG_Error("DSI: invalid data area!");
      return -L4_EINVAL;
    }

  LOGdL(DEBUG_SOCKET,"data_ds: %d at "l4util_idfmt, 
        data_ds->id, l4util_idstr(data_ds->manager));

  if (l4_is_invalid_id(work_id))
    {
      LOG_Error("DSI: invalid work thread");
      return -L4_EINVAL;
    }
  ...
  if (sid == -1)
    {
      ...
      LOG_Error("DSI: no socket available");
  ...
  if (l4_is_invalid_id(*sync_id))
      ...
      if (ret)
	{
	  ...
	  LOG_Error("DSI: create synchronization thread failed: %s (%d)",
  ...
  if (l4dm_is_invalid_ds(*ctrl_ds))
    {
      ...
      if(ret)
	{
	  LOG_Error("DSI: error creating control area: %s (%d)",
                    l4env_errstr(ret), ret);
	  return ret;
	}
      ...
      *ctrl_ds = s->ctrl_ds;
      ...
      LOGdL(DEBUG_SOCKET,"control dataspace: %d at "l4util_idfmt, 
            ctrl_ds->id, l4util_idstr(ctrl_ds->manager));
    }
  else 
    {
      /* use existing */
  ...
  if (ret)
    {
      ...
      LOG_Error("DSI: create/setup control area failed: %s (%d)",
                l4env_errstr(ret), ret);
  ...
  /* setup data area */
  LOGdL(DEBUG_SOCKET,"setting data area (%d at "l4util_idfmt")...", 
        data_ds->id, l4util_idstr(data_ds->manager));
  ...
  if (ret)
    {
      /* setup failed */
      LOG_Error("DSI: setup data area failed: %s (%d)",
                l4env_errstr(ret), ret);
      if (s->flags & DSI_SOCKET_FREE_SYNC)
	dsi_shutdown_sync_thread(s);
                   ...
                   const dsi_socket_ref_t * remote_socket)
{
  ...
  if (l4_is_invalid_id(remote_socket->work_th) || 
    ...
    {
      LOG_printf("work "l4util_idfmt", sync "l4util_idfmt"\n",
	     l4util_idstr(remote_socket->work_th),
             l4util_idstr(remote_socket->sync_th));
      LOG_Error("DSI: invalid data in socket descriptor");
      return -L4_EINVAL;
    }

  LOGdL(DEBUG_CONNECT,"connecting socket %d",socket->socket_id);
  LOGdL(DEBUG_CONNECT,"remote: %d, "l4util_idfmt", "l4util_idfmt,
  ...
  if (ret)
    {
      LOG_Error("DSI: connect: wakeup synchronization thread failed!");
      return ret;
    }
...
/*****************************************************************************/
/**
 * \brief   Return number of packets in ring list
 * \ingroup socket
 * 
 * \param   socket       Socket descriptor
 *	
 * \return  Number of packest in socket ring list (> 0), error code otherwise:
...
dsi_socket_get_packet_num(dsi_socket_t * socket)
{
  ...
  return socket->num_packets;
}

/*****************************************************************************/
/**
 * \brief   Return number of committed send packets in ring list
 * \ingroup socket
 *
 * \param   socket       Socket descriptor
 *	
 * \return  Number of committed send packets in ring list (> 0),
...
dsi_socket_get_num_committed_packets(dsi_socket_t * socket)
{
  ...
  return socket->header->packets_committed;
}
