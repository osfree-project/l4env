...
extern "C" int l4_qt_flips_available(void);
#endif
...
int QSocketDevice::createNewSocket()
{
    ...
    // If FLIPS is not running, we can't use IP sockets
    if (l4_qt_flips_available()) {
#if !defined(QT_NO_IPV6)
        s = qt_socket_socket( protocol() == IPv6 ? AF_INET6 : AF_INET,
                              t == Datagram ? SOCK_DGRAM : SOCK_STREAM, 0 );
#else
        s = qt_socket_socket( AF_INET, t==Datagram?SOCK_DGRAM:SOCK_STREAM, 0 );
#endif
    } else {
        s = qt_socket_socket(PF_LOCAL, SOCK_STREAM, 0);
    }
    if ( s < 0 ) {
	switch( errno ) {
