...
#else
#define DROPS_DSOUND 0 // use dsound or rely on advanced soundserver
#if DROPS_DSOUND
extern int drops_qws_sound_init(void);
extern int drops_qws_sound_feed(void *buffer, int length);
#else
extern int drops_qws_sound_play(const char *filename);
#endif
#endif
...
public slots:
...
#else
	// fire-n-forget IPC call to future DROPS advanced sound server
	drops_qws_sound_play(filename.latin1());
#endif // DROPS_SOUND
#endif // Q_OS_DROPS
    }
	...
	if ( !unwritten ) {
	    ...
	    int w = drops_qws_sound_feed((void*)left, blank);
