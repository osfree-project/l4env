...
void c_zoom (unsigned int *expix1, unsigned int *expix2, unsigned int prevX,
...
extern const void ppc_zoom (unsigned int *frompixmap, unsigned int *topixmap,
			    ...
			    unsigned int buffratio, int precalCoef[16][16]);
...
void
c_zoom (unsigned int *expix1, unsigned int *expix2, unsigned int prevX,
	unsigned int prevY, signed int *brutS, signed int *brutD)
{
  int myPos, myPos2;
  Color couleur;
//      unsigned int coefv, coefh;
  ...
  expix1[0] = expix1[prevX - 1] = expix1[prevX * prevY - 1] =
    expix1[prevX * prevY - prevX] = 0;
...
/* */
  ...
  if (firstTime || zf)
  {
    ...
    if (firstTime)
    {
      ...
      freebrutS =
	(unsigned int *) calloc (resx * resy * 2 + 128,
				 sizeof (unsigned int));
...
//        buffratio = 0;
  ...
  if (interlace_start == -1)
  {
	...
	brutSmypos + (((brutD[x] - brutSmypos) * buffratio) >> BUFFPOINTNB);
      brutSmypos = brutS[x2];
      brutS[x2] =
	brutSmypos + (((brutD[x2] - brutSmypos) * buffratio) >> BUFFPOINTNB);
    }
#endif
    buffratio = 0;
  }
