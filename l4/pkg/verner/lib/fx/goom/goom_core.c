...
*/
  ...
  if ((!abiggoom) && (speedvar > 4) && (goomlimit > 4) &&
      ((accelvar > goomlimit * 9 / 8 + 7)
       || (accelvar < -goomlimit * 9 / 8 - 7)))
  {
    int size, i;
    static int couleur =
      (0xc0 << (ROUGE * 8))
      | (0xc0 << (VERT * 8)) | (0xf0 << (BLEU * 8)) | (0xf0 << (ALPHA * 8));
    abiggoom = 100;
...
//        }
  ...
  if (lockvar == 0)
  {
	...
	if ((rndn >= states[i].rangemin) && (rndn <= states[i].rangemax))
	  curGState = states + i;
	...
	if ((zfd.mode == WATER_MODE)
	    || (zfd.mode == YONLY_MODE) || (zfd.mode == AMULETTE_MODE))
	{
	  zfd.middleX = resolx / 2;
	  zfd.middleY = c_resoly / 2;
	}
  ...
  if (goomlimit != 0)
    tentacle_update (p2 + c_offset, p1 + c_offset, resolx, c_resoly,
		     data, (float) accelvar / goomlimit,
		     curGState->drawTentacle);
  else
    tentacle_update (p2 + c_offset, p1 + c_offset, resolx, c_resoly,
   ...
   */
  {
	...
	goom_draw_text (p1 + c_offset, resolx, c_resoly,
      ...
      if (agoom == 20)
	goom_draw_text (p1 + c_offset, resolx, c_resoly, 10, 80, "GOOM", 1.0f,
			0);
      else if (agoom)
	goom_draw_text (p1 + c_offset, resolx, c_resoly, 10, 80, "goom", 1.0f,
			0);
      if (abiggoom == 200)
	goom_draw_text (p1 + c_offset, resolx, c_resoly, 10, 100, "BGOOM",
			1.0f, 0);
      else if (abiggoom)
	goom_draw_text (p1 + c_offset, resolx, c_resoly, 10, 100, "bgoom",
			1.0f, 0);
  ...
  else if ((cycle % 80 == 0) && (iRAND (5) == 0) && lineMode)
    lineMode--;

  if ((cycle % 120 == 0) && (iRAND (4) == 0) && (curGState->drawScope))
  {
    if (lineMode == 0)
      lineMode = DRAWLINES;
