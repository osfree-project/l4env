...
static int *font_pos;
static unsigned int current = 32;
...
gfont_load (void)
{
  ...
  gfont =
    malloc (the_font.width * the_font.height * the_font.bytes_per_pixel);
  ...
  for (i = 33; i < current; i++)
  {
    ...
    for (y = 0; y < font_height[i] / 2; y++)
    {
      ...
      {
	unsigned int r1, g1, b1, a1, r2, g2, b2, a2, r3, g3, b3, a3, r4, g4,
	  b4, a4;
	r1 =
	  gfont[2 * (y + 1) * (the_font.width * 4) +
		(x * 8 + font_pos[i] * 4)];
	g1 =
	  gfont[2 * (y + 1) * (the_font.width * 4) +
		(x * 8 + font_pos[i] * 4 + 1)];
...
void
goom_draw_text (guint32 * buf, int resolx, int resoly,
	...
	for (; yy < ymax; yy++)
	  for (xx = xmin; xx < xmax; xx++)
	  {
		...
		buf[yy * resolx + xx] =
		  (((((color & 0x00ff0000) * a1) +
		     ((back & 0x00ff0000) *
		      a2)) & 0xff000000) >> 8) | (((((color & 0x0000ff00) *
						     a1) +
						    ((back & 0x0000ff00) *
						     a2)) & 0x00ff0000) >> 8)
		  |
		  ((((color & 0x000000ff) * a1) +
		    ((back & 0x000000ff) * a2)) >> 8);
	      }
	    }
	  }
      }
