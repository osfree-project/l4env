...
//        printf("fps:%2d bps:%2d roundingType:%1d\n", fps, s->bit_rate/1024, s->flipflop_rounding);
    ...
    {
        printf( "I frame too long, ignoring ext header\n");
    }
                              ...
                              int n, int coded, const uint8_t *scan_table)
{
	...
	/* DC coef */
        ...
        if (level < 0){
            printf( "dc overflow- block: %d qscale: %d//\n", n, s->qscale);
        ...
        if (n < 4) {
            ...
            if(level > 256*s->y_dc_scale){
                printf( "dc overflow+ L qscale: %d//\n", s->qscale);
                if(!s->inter_intra_pred) return -1;
            }
        } else {
            rl = &rl_table[3 + s->rl_chroma_table_index];
            if(level > 256*s->c_dc_scale){
                printf( "dc overflow+ C qscale: %d//\n", s->qscale);
                if(!s->inter_intra_pred) return -1;
            }
        }
        block[0] = level;
...
//printf("level: %d, run: %d at %d %d\n", level, run, s->mb_x, s->mb_y);
		    ...
		    if (level>0) level= level * qmul + qadd;
                    ...
                    if(level>2048 || level<-2048){
                        printf( "|level| overflow in 3. esc\n");
                        return DECODING_AC_LOST;
                    }
#endif
                    i+= run + 1;
...
#ifdef ERROR_DETAILS
                if(run==66)
                    printf( "illegal vlc code in ESC3 level=%d\n", level);
                else if((i>62 && i<192) || i>192+63)
                    printf( "run overflow in ESC3 i=%d run=%d level=%d\n", i, run, level);
#endif
                } else {
                    /* second escape */
...
#ifdef ERROR_DETAILS
                if(run==66)
                    printf( "illegal vlc code in ESC2 level=%d\n", level);
                else if((i>62 && i<192) || i>192+63)
                    printf( "run overflow in ESC2 i=%d run=%d level=%d\n", i, run, level);
#endif
                }
            } else {
                /* first escape */
...
#ifdef ERROR_DETAILS
                if(run==66)
                    printf( "illegal vlc code in ESC1 level=%d\n", level);
                else if((i>62 && i<192) || i>192+63)
                    printf( "run overflow in ESC1 i=%d run=%d level=%d\n", i, run, level);
#endif
            }
        } else {
            i+= run;
...
#ifdef ERROR_DETAILS
                if(run==66)
                    printf( "illegal vlc code level=%d\n", level);
                else if((i>62 && i<192) || i>192+63)
                    printf( "run overflow i=%d run=%d level=%d\n", i, run, level);
#endif
        }
        if (i > 62){
            i-= 192;
            if(i&(~63)){
                ...
                if(((i+192 == 64 && level/qmul==-1) || s->error_resilience<=1) && left>=0){
                    printf( "ignoring overflow at %d %d\n", s->mb_x, s->mb_y);
                    break;
                }else{
