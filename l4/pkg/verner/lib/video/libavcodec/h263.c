        ...
        for(; mb_num < mb_count && s->mb_x<s->mb_width; s->mb_x++){
            ...
            if(s->pict_type==I_TYPE){
                ...
                if(IS_INTRA(s->current_picture.mb_type[xy])){          
                    ...
                    if(cbpy<0){
                        printf( "I cbpy corrupted at %d %d\n", s->mb_x, s->mb_y);
                        return -1;
                    }
                    ...
                    if(cbpy<0){
                        printf( "P cbpy corrupted at %d %d\n", s->mb_x, s->mb_y);
                        return -1;
                    }
...
int ff_mpeg4_decode_partitions(MpegEncContext *s)
{
    ...
    if(s->pict_type==I_TYPE){
        if(get_bits_long(&s->gb, 19)!=DC_MARKER){
            printf( "marker missing after first I partition at %d %d\n", s->mb_x, s->mb_y);
            return -1;
        }
    }else{
        if(get_bits(&s->gb, 17)!=MOTION_MARKER){
            printf( "marker missing after first P partition at %d %d\n", s->mb_x, s->mb_y);
            return -1;
        }
    }
    ...
    if (!IS_SKIP(mb_type)) {
        ...
        for (i = 0; i < 6; i++) {
            if(mpeg4_decode_block(s, block[i], i, cbp&32, s->mb_intra, s->rvlc) < 0){
                printf( "texture corrupted at %d %d %d\n", s->mb_x, s->mb_y, s->mb_intra);
                return -1;
            }
            cbp+=cbp;
        }
                             ...
                             int n, int coded)
{
    ...
    for(;;) {
        ...
        if (code < 0){
            printf( "illegal ac vlc code at %dx%d\n", s->mb_x, s->mb_y);
            return -1;
        }
        if (code == rl->n) {
            /* escape */
        ...
        if (level==0) {
          ...
          if(rvlc){
                if(SHOW_UBITS(re, &s->gb, 1)==0){
                    printf( "1. marker bit missing in rvlc esc\n");
                    return -1;
                ...
                if(SHOW_UBITS(re, &s->gb, 1)==0){
                    printf( "2. marker bit missing in rvlc esc\n");
                    return -1;
                }; SKIP_CACHE(re, &s->gb, 1);
            ...
            if (cache&0x80000000) {
                if (cache&0x40000000) {
                    ...
                    if(IS_3IV1){
                        ...
                        if(SHOW_UBITS(re, &s->gb, 1)==0){
                            printf( "1. marker bit missing in 3. esc\n");
                            return -1;
                        }; SKIP_CACHE(re, &s->gb, 1);
                        ...
                        if(SHOW_UBITS(re, &s->gb, 1)==0){
                            printf( "2. marker bit missing in 3. esc\n");
                            return -1;
                    ...
                    if(level*s->qscale>1024 || level*s->qscale<-1024){
                        printf( "|level| overflow in 3. esc, qp=%d\n", s->qscale);
                        return -1;
                    }
#if 1 
                    {
                        ...
                        if(abs_level<=MAX_LEVEL && run<=MAX_RUN && !(s->workaround_bugs&FF_BUG_AC_VLC)){
                            ...
                            if(abs_level <= rl->max_level[last][run]){
                                printf( "illegal 3. esc, vlc encoding possible\n");
                                return -1;
                            }
                            if(s->error_resilience > FF_ER_COMPLIANT){
                                if(abs_level <= rl->max_level[last][run]*2){
                                    printf( "illegal 3. esc, esc 1 encoding possible\n");
                                    return -1;
                                }
                                if(run1 >= 0 && abs_level <= rl->max_level[last][run1]){
                                    printf( "illegal 3. esc, esc 2 encoding possible\n");
                                    return -1;
                                }
                            }
                        }
...
int h263_decode_picture_header(MpegEncContext *s)
{
    ...
    */
    ...
    if (format != 7 && format != 6) {
        ...
        if (get_bits1(&s->gb) != 0) {
            printf( "H263 PB frame not supported\n");
        ...
        if (ufep == 1) {
            ...
            if (get_bits1(&s->gb) != 0) {
                printf( "Deblocking Filter not supported\n");
            }
            if (get_bits1(&s->gb) != 0) {
                printf( "Slice Structured not supported\n");
            }
            if (get_bits1(&s->gb) != 0) {
            ...
            if (get_bits1(&s->gb) != 0) {
                printf( "Alternative Inter VLC not supported\n");
            }
            if (get_bits1(&s->gb) != 0) {
                printf( "Modified Quantization not supported\n");
            }
...
//                printf( "illegal scalability header (VERY broken encoder), trying to workaround\n");
                s->scalability=0;
...
//        printf( "time_increment_resolution is illegal\n");
    }
    time_incr=0;
...
//                printf( "header is not mpeg4 compatible, broken encoder, trying to workaround\n");
                s->time_base++;
...
int intel_h263_decode_picture_header(MpegEncContext *s)
{
    ...
    format = get_bits(&s->gb, 3);
    if (format != 7) {
        printf( "Intel H263 free format not supported\n");
        return -1;
    }
    s->h263_plus = 0;
    ...
    if (get_bits1(&s->gb) != 0) {
        printf( "SAC not supported\n");
        return -1;	/* SAC: off */
    }
    if (get_bits1(&s->gb) != 0) {
        printf( "Advanced Prediction Mode not supported\n");
        return -1;	/* advanced prediction mode: off */
    }
    if (get_bits1(&s->gb) != 0) {
        printf( "PB frame mode no supported\n");
