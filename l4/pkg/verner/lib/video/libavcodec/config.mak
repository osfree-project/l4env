...
ifeq ($(BUILD_ARCH),x86)
OPTFLAGS=-O3 -g -mcpu=i686
SHCFLAGS=-O3 -g -mcpu=i686
TARGET_ARCH_X86=yes
TARGET_MMX=yes
TARGET_BUILTIN_VECTOR=no
else
# FIXME: one day we may want to support additional architectures like ARM
TARGET_ARCH_UNSUPPORTED=yes
endif

ifeq ($(BUILD_libavcodec_encoder),y)
CONFIG_ENCODERS=yes
CONFIG_MUXERS=yes
endif
ifeq ($(BUILD_libavcodec_decoder),y)
CONFIG_DECODERS=yes
CONFIG_DEMUXERS=yes
endif

CONFIG_VIDEO4LINUX=no
CONFIG_VIDEO4LINUX2=no
CONFIG_DV1394=no
CONFIG_AUDIO_OSS=no
CONFIG_NETWORK=no
CONFIG_ZLIB=no
