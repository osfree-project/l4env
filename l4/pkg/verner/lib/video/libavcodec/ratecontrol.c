...
int ff_rate_control_init(MpegEncContext *s)
{
    ...
    if(s->flags&CODEC_FLAG_PASS2){
        ...
        for(i=0; i<rcc->num_entries - s->max_b_frames; i++){
            ...
            if(e!=12){
                printf( "statistics are damaged at line %d, parser out=%d\n", i, e);
                return -1;
            }
            p= next;
        }
...
static void update_rc_buffer(MpegEncContext *s, int frame_size){
    ...
    if(buffer_size){
        ...
        if(rcc->buffer_index < 0)
            printf( "rc buffer underflow\n");
        if(rcc->buffer_index >= s->avctx->rc_buffer_size)
            printf( "rc buffer overflow\n");
    }
}
...
static int init_pass2(MpegEncContext *s)
{
    ...
    if(all_available_bits < all_const_bits){
        printf( "requested bitrate is to low\n");
        return -1;
    }
...
//        printf("%f %d %f\n", expected_bits, (int)all_available_bits, rate_factor);
    ...
    if(abs(expected_bits/all_available_bits - 1.0) > 0.01 ){
        printf( "Error: 2pass curve failed to converge\n");
        return -1;
    }
