...
static void* dlsymm(void* handle, const char* symbol)
{
    ...
    if (!f)
	printf((stderr, "A52 Decoder - function '%s' can't be resolved\n", symbol);
    return f;
}
#endif
...
static int a52_decode_init(AVCodecContext *avctx)
{
    ...
    if (!s->handle)
    {
	printf((stderr, "A52 library %s could not be opened! \n%s\n", liba52name, dlerror());
        return -1;
    }
    ...
    *data_size = 0;
    ...
    while (buf_size > 0) {
        ...
        if (s->frame_size == 0) {
			...
			printf((stderr, "ac3dec: AC3 Source channels are less than specified: output to %d channels.. (frmsize: %d)\n", s->channels, len);
			avctx->channels = s->channels;
		    }
