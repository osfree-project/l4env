...
static inline int check_intra4x4_pred_mode(H264Context *h){
    ...
    if(!(h->top_samples_available&0x8000)){
        for(i=0; i<4; i++){
            ...
            if(status<0){
                printf("top block unavailable for requested intra4x4 mode %d at %d %d\n", status, s->mb_x, s->mb_y);
                return -1;
            } else if(status){
    ...
    if(!(h->left_samples_available&0x8000)){
        for(i=0; i<4; i++){
            ...
            if(status<0){
                printf("left block unavailable for requested intra4x4 mode %d at %d %d\n", status, s->mb_x, s->mb_y);
                return -1;
            } else if(status){
...
static inline int check_intra_pred_mode(H264Context *h, int mode){
    ...
    if(!(h->top_samples_available&0x8000)){
        mode= top[ mode ];
        if(mode<0){
            printf("top block unavailable for requested intra mode at %d %d\n", s->mb_x, s->mb_y);
            return -1;
        }
    }
    ...
    if(!(h->left_samples_available&0x8000)){
        mode= left[ mode ];
        if(mode<0){
            printf("left block unavailable for requested intra mode at %d %d\n", s->mb_x, s->mb_y);
            return -1;
        } 
    }
                ...
                if(reordering_of_pic_nums_idc<3){
                    if(reordering_of_pic_nums_idc<2){
                        ...
                        if(abs_diff_pic_num >= h->max_pic_num){
                            printf("abs_diff_pic_num overflow\n");
                            return -1;
                        }
                    ...
                    if(i < index){
                        printf("reference picture missing during reorder\n");
                    ...
                    printf("illegal reordering_of_pic_nums_idc\n");
                    return -1;
                }
            }
        }
...
static int execute_ref_pic_marking(H264Context *h, MMCO *mmco, int mmco_count){
    ...
    if(!current_is_long){
        ...
        if(pic){
            pic->reference=0;
            printf("illegal short term buffer state detected\n");
        }
...
/*                    if(h->mmco[i].short_frame_num >= h->short_ref_count || h->short_ref[ h->mmco[i].short_frame_num ] == NULL){
                        printf("illegal short ref in memory management control operation %d\n", mmco);
                        return -1;
                    }*/
                }
...
static int decode_slice_header(H264Context *h){
    ...
    if(h->slice_type > 9){
        printf("slice type too large (%d) at %d %d\n", h->slice_type, s->mb_x, s->mb_y);
    }
    if(h->slice_type > 4){
        h->slice_type -= 5;
    ...
    if(pps_id>255){
        printf("pps_id out of range\n");
        return -1;
    }
    h->pps= h->pps_buffer[pps_id];
    if(h->pps.slice_group_count == 0){
        printf("non existing PPS referenced\n");
        return -1;
    }

    h->sps= h->sps_buffer[ h->pps.sps_id ];
    if(h->sps.log2_max_frame_num == 0){
        printf("non existing SPS referenced\n");
        return -1;
    }
...
static int decode_residual(H264Context *h, GetBitContext *gb, DCTELEM *block, int n, const uint8_t *scantable, int qp, int max_coeff){
    ...
    for(; i<total_coeff; i++){
        ...
        if(prefix<14){ //FIXME try to build a large unified VLC table for all this
            ...
            printf("prefix too large at %d %d\n", s->mb_x, s->mb_y);
            return -1;
        }
    ...
    if(zeros_left<0){
        printf("negative number of zero coeffs at %d %d\n", s->mb_x, s->mb_y);
        return -1;
    }
...
decode_intra_mb:
        if(mb_type > 25){
            printf("mb_type %d in %c slice to large at %d %d\n", mb_type, av_get_pict_type_char(h->slice_type), s->mb_x, s->mb_y);
            return -1;
        }
        partition_count=0;
        ...
        if(h->slice_type == B_TYPE){
            for(i=0; i<4; i++){
                ...
                if(h->sub_mb_type[i] >=13){
                    printf("B sub_mb_type %d out of range at %d %d\n", h->sub_mb_type[i], s->mb_x, s->mb_y);
                    return -1;
                }
            ...
            for(i=0; i<4; i++){
                ...
                if(h->sub_mb_type[i] >=4){
                    printf("P sub_mb_type %d out of range at %d %d\n", h->sub_mb_type[i], s->mb_x, s->mb_y);
                    return -1;
                }
...
//        fill_non_zero_count_cache(h);
        ...
        if( dquant > 25 || dquant < -26 ){
            printf("dquant out of range (%d) at %d %d\n", dquant, s->mb_x, s->mb_y);
            return -1;
        }
...
static int decode_slice(H264Context *h){
    ...
    for(;;){
        ...
        if(ret<0){
            printf("error while decoding MB %d %d\n", s->mb_x, s->mb_y);
...
static inline int decode_seq_parameter_set(H264Context *h){
    ...
    if(sps->crop){
        ...
        if(sps->crop_left || sps->crop_top){
            printf("insane croping not completly supported, this could look slightly wrong ...\n");
        }
    }else{
        sps->crop_left  = 
        sps->crop_right = 
