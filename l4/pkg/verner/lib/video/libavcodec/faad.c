...
static int faac_init_mp4(AVCodecContext *avctx)
{
    ...
    if (r < 0)
	printf("faacDecInit2 failed r:%d   sr:%ld  ch:%d  s:%d\n",
                             ...
                             uint8_t *buf, int buf_size)
{
	...
	printf("faac: frame decodinf failed: %s\n",
...
static int faac_decode_init(AVCodecContext *avctx)
{
    ...
    if (!s->handle)
    {
	printf("FAAD library: %s could not be opened! \n%s\n",
		libfaadname, dlerror());
        return -1;
    }
#define dfaac(a, b) \
...
#undef dfacc
    ...
    if (err) {
        dlclose(s->handle);
	printf("FAAD library: cannot resolve %s in %s!\n",
		err, libfaadname);
        return -1;
    }
#endif
    ...
    if (!s->faac_handle) {
        printf( "FAAD library: cannot create handler!\n");
	...
	case 8: printf( "FAADlib unsupported bps %d\n", avctx->bits_per_sample); break;
	default:
	case 16:
