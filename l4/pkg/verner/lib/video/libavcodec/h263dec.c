...
//       &&   !s->resync_marker
    ...
    if(s->msmpeg4_version || (s->workaround_bugs&FF_BUG_NO_PADDING)){ //FIXME perhaps solve this more cleanly
        ...
        if(left>max_extra){
            printf("discarding %d junk bits at end, next would be %X\n", left, show_bits(&s->gb, 24));
        }
        else if(left<0){
            printf("overreading %d bits\n", -left);
        }else
            ff_er_add_slice(s, s->resync_mb_x, s->resync_mb_y, s->mb_x-1, s->mb_y, AC_END|DC_END|MV_END);
    ...
    printf("slice end not reached but screenspace end (%d left %06X, score= %d)\n", 
    ...
    *data_size = 0;
    ...
    if(s->flags&CODEC_FLAG_TRUNCATED){
        ...
        if(s->codec_id==CODEC_ID_MPEG4){
            ...
            printf( "this codec doesnt support truncated bitstreams\n");
            return -1;
        }
