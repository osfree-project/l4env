...
void* tmp;
	...
	if(tmp){
	    if(a52_resample==NULL) printf("Using MMX optimized resampler\n");
	    a52_resample=tmp;
	    return tmp;
	}
    }
    ...
    if(tmp){
	if(a52_resample==NULL) printf("No accelerated resampler found\n");
	a52_resample=tmp;
	return tmp;
    }
    
    printf("Unimplemented resampler for mode 0x%X -> %d channels conversion - Contact MPlayer developers!\n", flags, chans);
    return NULL;
}
