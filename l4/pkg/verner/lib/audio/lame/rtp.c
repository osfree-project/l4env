...
int makesocket(char *szAddr,unsigned short port,int TTL,struct sockaddr_in *sSockAddr) {
  ...
  if (iRet < 0) {
    printf("lame: setsockopt SO_REUSEADDR failed\n");
    exit(1);
  }
  ...
  if ((ntohl(tempaddr) >> 28) == 0xe) {
    ...
    if (iRet < 0) {
      printf("lame: setsockopt IP_MULTICAST_TTL failed.  multicast in kernel?\n");
      exit(1);
    }
    ...
    iRet = setsockopt(iSocket, IPPROTO_IP, IP_MULTICAST_LOOP,
		      &cLoop, sizeof(char));
    if (iRet < 0) {
      printf("lame: setsockopt IP_MULTICAST_LOOP failed.  multicast in kernel?\n");
      exit(1);
    }
  }
