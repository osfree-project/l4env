...
void display_bitrates(FILE *out_fh)
{
  ...
  version = 1;
  printf("\n");
  printf("MPEG1 samplerates(kHz): 32 44.1 48 \n");

  printf("bitrates(kbs): ");
  for (index=1;index<15;index++) {
    printf("%i ",bitrate_table[version][index]);
  }
  printf("\n");
  ...
  version = 0;
  printf("\n");
  printf("MPEG2 samplerates(kHz): 16 22.05 24 \n");
  printf("bitrates(kbs): ");
  for (index=1;index<15;index++) {
    printf("%i ",bitrate_table[version][index]);
  }
...
int     found = 0;
    ...
    if(found)
        ...
        printf("lame: Bitrate %dkbs not legal for %iHz output sampling.\n",
                bRate, samplerate);
        return(-1);     /* Error! */
    }
}
	...
	*version=0;
    ...
    if (sRate == 44100L) {
        ...
        *version = 0; return(2);
        ...
        printf("lame: SmpFrqIndex: %ldHz is not a legal sample rate\n", sRate);
        return(-1);     /* Error! */
    }
}
...
void  *mem_alloc(unsigned long block, char *item)
{
    ...
    if (ptr != NULL) {
        ...
        printf("lame: Unable to allocate %s\n", item);
        //exit(1); cr7 - may fail
	return NULL;
    }
    return(ptr);
...
int N)                  /* number of bits of val */
{
 ...
 if (N > MAX_LENGTH)
    printf("lame: Cannot read or write more than %d bits at a time.\n", MAX_LENGTH);
