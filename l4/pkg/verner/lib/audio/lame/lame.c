...
/********************************************************************
 *   cr7: adjust quality while working. this may crash
 *   
 *
 ********************************************************************/
void lame_change_quality(lame_global_flags *gfp)
{
...
void lame_init_params(lame_global_flags *gfp)
{
  ...
  */
  ...
  if (gfp->filter_type==0) {
	...
	printf("lame: Warning: highpass filter disabled.  highpass frequency to small\n");
      }
...
//cr7      InitVbrTag(&bs,1-gfp->version,gfp->mode,gfp->samplerate_index);
    }
...
void lame_print_config(lame_global_flags *gfp)
{
  ...
  lame_print_version(2);
  if (gfp->num_channels==2 && gfp->stereo==1) {
    printf("lame: Autoconverting from stereo to mono. Setting encoding to mono mode.\n");
  }
  if (gfp->resample_ratio!=1) {
    printf("lame: Resampling:  input=%ikHz  output=%ikHz\n",
	    (int)in_samplerate,(int)out_samplerate);
  }
  if (gfp->highpass2>0.0)
    printf("lame: Using polyphase highpass filter, transition band: %.0f Hz -  %.0f Hz\n",
	    gfp->highpass1*out_samplerate*500,
	    gfp->highpass2*out_samplerate*500);
  if (gfp->lowpass1>0.0)
    printf("lame: Using polyphase lowpass filter,  transition band:  %.0f Hz - %.0f Hz\n",
	    gfp->lowpass1*out_samplerate*500,
	    gfp->lowpass2*out_samplerate*500);
/*
  if (gfp->gtkflag) {
    printf("lame: Analyzing %s \n",gfp->inPath);
  }
  else {
    printf("lame: Encoding %s to %s\n",
	    (strcmp(gfp->inPath, "-")? gfp->inPath : "stdin"),
	    (strcmp(gfp->outPath, "-")? gfp->outPath : "stdout"));
    if (gfp->VBR)
      printf("lame: Encoding as %.1fkHz VBR(q=%i) %s MPEG%i LayerIII  qval=%i\n",
	      gfp->out_samplerate/1000.0,
	      gfp->VBR_q,mode_names[gfp->mode],2-gfp->version,gfp->quality);
    else
      printf("lame: Encoding as %.1f kHz %d kbps %s MPEG%i LayerIII (%4.1fx)  qval=%i\n",
	      gfp->out_samplerate/1000.0,gfp->brate,
	      mode_names[gfp->mode],2-gfp->version,compression,gfp->quality);
  }
*/
#if 0
     printf("lame: Encoding as %.1f kHz %d kbps %s MPEG%i LayerIII (%4.1fx)  qval=wil be set later.\n",
	      gfp->out_samplerate/1000.0,gfp->brate,
	      mode_names[gfp->mode],2-gfp->version,compression);
#endif
/* double ouput --> int output ! */
     printf("lame: Encoding as %d Hz %d kbps %s MPEG%i LayerIII (%4.1fx)  qval=wil be set later.\n",
	      (int)gfp->out_samplerate,gfp->brate,
	      mode_names[gfp->mode],2-gfp->version,compression);
...
int mf_size,char *mp3buf, int mp3buf_size)
{
  ...
  */
  ...
  if ( frameBits % bitsPerSlot )   /* a program failure */
    printf("lame: Sent %ld bits = %ld slots plus %ld\n",
...
//cr7  if (gfp->bWriteVbrTag) AddVbrFrame((int)(sentBits/8));
...
# include <floatingpoint.h>
  ...
  /*  printf("lame: FreeBSD mask is 0x%x\n",mask); */
  }
#endif
