...
/* the second index is the "age" of the data.                             */
  ...
  if((gfp->frameNum==0) && (gr_out==0)){
    ...
    case 22050: break;
    case 24000: break;
    default:    printf("lame: error, invalid sampling frequency: %d Hz\n",i);
      exit(-1);
    }
  ...
  for (chn=0; chn<gfp->stereo; chn++) {
	...
	switch( blocktype_old[chn] ) 
	  ...
	  case SHORT_TYPE:
	    ...
	    break;
	  case START_TYPE:
	    printf("lame: Error in block selecting\n" );
	    abort();
	    break; /* problem */
	  }
      } else   {
	  ...
	  for(i=0,k2=0;i<cbmax_tp;i++)
	    {
	      ...
	      if (j!=i)
		{
		  printf("lame: 1. please check \"psy_data\"");
		  exit(-1);
		}
	  ...
	  for(i=0,k2=0;i<cbmax_tp;i++)
	    {
	      ...
	      if (j!=i)
		{
		  printf("lame: 3. please check \"psy_data\"");
		  exit(-1);
		}
	      numlines_s[i]--;
	    }
	  ...
	  for(i=0;i<sbmax;i++)
	    {
	      ...
	      w2_l[i] = (FLOAT8) *p++;
	      if (j!=i)
		{ printf("lame: 30:please check \"psy_data\"\n");
		exit(-1);
		}
		...
		if ( (fabs(1.0-w1_l[i]-w2_l[i-1]) > 0.01 ) )
		  {
		    printf("lame: 31l: please check \"psy_data.\"\n");
                  printf("lame: w1,w2: %f %f \n",w1_l[i],w2_l[i-1]);
		    exit(-1);
		  }
	    }
	}
      ...
      if (sfreq == freq_tp/freq_scale)
	{
	  for(i=0;i<sbmax;i++)
		...
		if ( (fabs(1.0-w1_s[i]-w2_s[i-1]) > 0.01 ) )
		  { 
                  printf("lame: 31s: please check \"psy_data.\"\n");
                  printf("lame: w1,w2: %f %f \n",w1_s[i],w2_s[i-1]);
		  exit(-1);
		  }
	    }
