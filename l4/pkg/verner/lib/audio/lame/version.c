...
void lame_print_version(FILE *ofile) {
  printf("LAME version %s (www.sulaco.org/mp3) \n",get_lame_version());
  printf("GPSYCHO: GPL psycho-acoustic and noise shaping model version %s. \n",get_psy_version());
#ifdef LIBSNDFILE
  printf("Input handled by libsndfile (www.zip.com.au/~erikd/libsndfile)\n");
#endif
}
