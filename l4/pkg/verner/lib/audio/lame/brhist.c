...
void brhist_init(lame_global_flags *gfp,int br_min, int br_max)
{
  ...
  if ((termname = getenv("TERM")) == NULL)
    {
      printf("lame: can't get TERM environment string.\n");
      disp_brhist = 0;
      return;
    }
  ...
  if (tgetent(term_buff, termname) != 1)
    {
      printf("lame: can't find termcap entry: %s\n", termname);
      disp_brhist = 0;
      return;
    }
...
#if 0
  printf("lame: ----- bitrate statistics -----\n");
  printf("lame:  [kbps]      frames\n");
  for(i = brhist_vbrmin; i <= brhist_vbrmax; i++)
    {
      printf("lame:    %3d  %8ld (%.1f%%)\n",
