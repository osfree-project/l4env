		...
		int osize,int *done)
{
	...
	if(osize < 4608) {
		printf("mpglib: To less out space\n");
		return MP3_ERR;
	}
	...
	if(mp->framesize == 0) {
		...
		read_head(mp);
		// decode_header(&mp->fr,mp->header cr7 - patched
...
int set_pointer(long backstep)
{
  ...
  if(gmp->fsizeold < 0 && backstep > 0) {
    printf("mpglib: Can't step back %ld!\n",backstep);
    return MP3_ERR;
  }
