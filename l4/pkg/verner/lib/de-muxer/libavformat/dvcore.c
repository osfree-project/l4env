...
void dv_assemble_frame(DVMuxContext *c, const uint8_t* video, const uint8_t* audio, int asize)
{
	...
	if (c->has_video)
	    printf("Can't process DV frame #%d. Insufficient audio data or severe sync problem.\n", c->frames);
	...
	if (fsize + asize >= reqasize) {
	    ...
	    printf("Can't process DV frame #%d. Insufficient video data or severe sync problem.\n", c->frames);
