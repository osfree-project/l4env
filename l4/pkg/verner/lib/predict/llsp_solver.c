#define COMPILE \
  gcc -O3 -Wall -Wstrict-prototypes -Wmissing-prototypes -Wmissing-declarations -Wno-long-long -ansi -pedantic -DTESTING -DTEST_MAIN -lm -o llsp_solver $0 ; exit
...
#  undef rand
#  define rand() l4util_rand()
#  undef RAND_MAX
#  define RAND_MAX L4_RAND_MAX
#endif

#include "predict.h"

/* FIXME: the amount of metrics values is currently limited, because a bitfield
...
double *llsp_load(llsp_t *llsp, const char *filename)
{
  ...
  do {
    ...
    if (fd < 0) {
      LOG("opening error, coefficients could not be loaded");
      break;
    }
    
    offset = MAX_METRICS * llsp->id * sizeof(double);
    if (lseek(fd, offset, SEEK_SET) != offset) {
      LOG("seeking error, coefficients could not be loaded");
      break;
    }
	...
	LOG("reading error, coefficients could not be loaded");
	break;
      }
      llsp->result[i] = value;
    }
...
int llsp_store(llsp_t *llsp, const char *filename)
{
  ...
  do {
    ...
    if (fd < 0) {
      LOG("opening error, coefficients could not be stored");
      break;
    }
    
    offset = MAX_METRICS * llsp->id * sizeof(double);
    if (lseek(fd, offset, SEEK_SET) != offset) {
      LOG("seeking error, coefficients could not be stored");
      break;
    }
	...
	LOG("writing error, coefficients could not be stored");
	break;
      }
    }
...
{
  /* FIXME: If llsp_finalize() is called by a background thread, the dispose
   * should be deferred until finalize finishes */
...
static int llsp_solve(llsp_t *llsp)
{
  ...
  /* FIXME: since (llsp->columns - 1) can be as large as the number of bits in
  ...
  /* FIXME: If llsp_finalize() is called by a background thread (so there is
   * potentially a prediction from previous results running right now), the
   * results should be locked before being written to */
...
int main(void)
{
  ...
  /* we always want the same pseudo-random numbers for reproducibility */
  srand(1234);
  
  llsp = llsp_new(0, 5);
  while (!feof(stdin)) {
    fscanf(stdin, "%lf,%lf,%lf,%lf,%lf,%lf\n",
      &metrics[0], &metrics[1], &metrics[2], &metrics[3], &metrics[4], &time);
    llsp_accumulate(llsp, metrics, time);
  }
  if ((result = llsp_finalize(llsp)))
    printf("%G\n%G\n%G\n%G\n%G\n",
