...
/*****************************************************************************/
/**
 * \file	dde_linux26/lib/src/res.c
 *
				...
				const char *name)
{
  ...
  LOGdL(DEBUG_RES_TRACE, "io_addr=0x%04lx, size=%ld, name=\"%s\"", start, n, name);
  err = l4io_request_region((l4_addr_t) start, (l4_size_t) n);

  if (err)
    {
      LOG_Error("in l4io_request_region(0x%04lx, %ld) (%d)", start, n, err);
      return 0;
    }
				    ...
				    const char *name)
{
  ...
  if (!vaddr)
    {
      LOG_Error("in l4io_request_mem_region(%p, %ld)", (void*)start, n);
      return 0;
    }
  /* keep region info */
...
void release_region(unsigned long start, unsigned long n)
{
  ...
  LOGdL(DEBUG_RES_TRACE, "io_addr=0x%04lx, size=%ld", start, n);
...
void release_mem_region(unsigned long start, unsigned long n)
{
  ...
  if(err)
    {
      LOG_Error("release_mem_region(%p, %ld) failed (%d)", (void*)start, n, err);
      return;
    }
