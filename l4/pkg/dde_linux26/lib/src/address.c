      ...
      if ((p->pa <= paddr) && (p->pa + p->size > paddr))
        {
          ...
          LOGd(DEBUG_ADDRESS, "__va(0x%lx) => %p", paddr, pp);
          return pp;
        }
      p = p->next;
    }
  ...
  while (p)
    {
      if ((p->va <= addr) && (p->va + p->size > addr))
        {
          pp = p->pa + (addr - p->va);
          LOGd(DEBUG_ADDRESS, "__pa(0x%lx) => 0x%lx", addr, pp);
          return pp;
        }
      p = p->next;
    }
