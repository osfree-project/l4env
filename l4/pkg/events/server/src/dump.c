...
dump_task_list(task_item_t *curr_task)
{
  ...
  {
    printf("task: %x channels: %i blocked: %i pending: %i ack: %i \n", 
	curr_task->taskid, 
...
dump_ack_list(ack_item_t *curr_notify_item)
{
  ...
  while (curr_notify_item != NULL)
  {
    printf("notify task "l4util_idfmt" for (%i,%i)\n",
...
dump_timeout_queue(event_item_t *timeout_curr_event)
{
  ...
  while (timeout_curr_event != NULL)
  {
    printf("timeout for event: (%i,%i) timeout:%i \n", 
