...
{
  LOGd(DEBUGLVL(2), "unlink event (%i.%i) from timeout queue",
  ...
  {
    i->timeout_prev_event->timeout_next_event =
      i->timeout_next_event;
  }
  else
  {
    timeout_first_event = 
  ...
  if (i->timeout_next_event != NULL)
  {
    i->timeout_next_event->timeout_prev_event =
...
link_event_to_timeout_queue(event_item_t *i)
{
  ...
  LOGd(DEBUGLVL(2), "link event (%i.%i) to timeout queue",
      i->event_ch, i->event_nr);
