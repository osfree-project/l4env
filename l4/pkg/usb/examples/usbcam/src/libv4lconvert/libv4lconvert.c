...
{
  int i, j;
  struct v4lconvert_data *data = malloc( sizeof(struct v4lconvert_data));
...
int v4lconvert_enum_fmt(struct v4lconvert_data *data, struct v4l2_fmtdesc *fmt)
{
  ...
  if (fmt->type != V4L2_BUF_TYPE_VIDEO_CAPTURE ||
    ...
    return ioctl(data->fd, VIDIOC_ENUM_FMT, fmt);
  ...
  struct v4l2_format *dest_fmt, struct v4l2_format *src_fmt)
{
  ...
  for (i = 0; i < ARRAY_SIZE(supported_src_pixfmts); i++) {
    ...
    if (!ioctl(data->fd, VIDIOC_TRY_FMT, &try_fmt))
    {
  ...
  struct v4l2_format *dest_fmt, struct v4l2_format *src_fmt)
{
  ...
  if (!v4lconvert_supported_dst_format(dest_fmt->fmt.pix.pixelformat) ||
    ...
    result = ioctl( data->fd, VIDIOC_TRY_FMT, dest_fmt);
    if (src_fmt)
      *src_fmt = *dest_fmt;
    return result;
  }
  ...
  unsigned int pixelformat, int index)
{
  ...
  for (i = 0; ; i++) {
    frmsize.index = i;
    if (ioctl( data->fd, VIDIOC_ENUM_FRAMESIZES, &frmsize))
      break;
  ...
  struct v4l2_frmsizeenum *frmsize)
{
  if (!v4lconvert_supported_dst_format(frmsize->pixel_format))
    return ioctl( data->fd, VIDIOC_ENUM_FRAMESIZES, frmsize);
  ...
  struct v4l2_frmivalenum *frmival)
{
  ...
  if (!v4lconvert_supported_dst_format(frmival->pixel_format)) {
    res = ioctl( data->fd, VIDIOC_ENUM_FRAMEINTERVALS, frmival);
    if (res)
      V4LCONVERT_ERR("%s\n", strerror(errno));
    return res;
  }
