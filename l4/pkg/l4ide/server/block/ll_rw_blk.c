...
void blk_recount_segments(request_queue_t *q, struct bio *bio)
{
	...
	bio_for_each_segment(bv, bio, i) {
		 ...
		 */
		high = page_to_pfn(bv->bv_addr) >= q->bounce_pfn;
		if (high || highprv)
			goto new_hw_segment;
		if (cluster) {
...
new_segment:
				...
				sg[nsegs].dma_address = (dma_addr_t)bvec->bv_addr;
...
void blk_start_queue(request_queue_t *q)
{
	...
	/*
	 * one level recursion is ok and is much faster than kicking
	 * the unplug handling
	 */
...
/*
	...
	blk_put_request(rq);
*/	printk("Fixme! blk_rq_unmap_user\n");
	return ret;
}
...
//__generic_unplug_device(q); for testing only
	spin_unlock_irq(q->queue_lock);
	return 0;
...
void generic_make_request(struct bio *bio)
{
	...
	maxsector = bio->bi_bdev->bd_size >> 9;
	if (maxsector) {
...
void submit_bio(int rw, struct bio *bio)
{
	...
	bio->bi_rw = rw;
/* Are our pages really locked? If not adapt this behavior!
	if (rw & WRITE)
...
int blk_register_queue(struct gendisk *disk)
{
	...
	q->parent = disk;
	// evtl noch ein busy check der disk

	snprintf(q->name, KOBJ_NAME_LEN, "%s", "queue");

	ret = elv_register_queue(q);
	if (ret) {
		// kobject_unregister(&q->kobj);
		// dieses evtl durch refcount emulieren
		return ret;
	}
