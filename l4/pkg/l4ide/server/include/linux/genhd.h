...
#else /* DDE_IDE */
#define add_disk_randomness(x)
#define rand_initialize_disk(x)
#endif /* DDE_IDE */
...
#else /* DDE_IDE */
extern int get_disk_num(void);

static inline struct block_device *bdget_disk(struct gendisk *disk, int index, int create_new)
{
	return bdget(MKDEV(disk->major, disk->first_minor) + index, create_new);
}
#endif /* DDE_IDE */
