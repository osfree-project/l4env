...
/*\brief Pass IRQ to other thread
 *
...
void server(void){
	...
	case	OMEGA0_ATTACH:
	    ...
	    break;
	case	OMEGA0_DETACH:
	    ret = detach(client, (omega0_irqdesc_t)dw1);
	    break;
	case	OMEGA0_PASS:
	    ret = pass(client, (omega0_irqdesc_t)dw1,
		       msg.new_thread);
	    break;
	case	OMEGA0_FIRST:
	    ret = first();
