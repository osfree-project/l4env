...
static void arm_lcd_h3800_area_set(struct arm_lcd_ops *s,
...
static void arm_lcd_h3800_area_put_pixel16(struct arm_lcd_ops *s,
                                           unsigned int val16)
{
}
...
static int arm_lcd_h3800_probe(void)
{
  // Weak detection but should work in our pool of supported hardware
  const char *s = l4sigma0_kip_version_string();
  if (strstr(s, "XScale") || strstr(s, "StrongARM"))
    return 1;
...
struct arm_lcd_ops arm_lcd_ops_h3800 = {
  ...
  .get_screen_width   = arm_lcd_h3800_get_screen_width,
  .get_screen_height  = arm_lcd_h3800_get_screen_height,
  .get_bpp            = arm_lcd_h3800_get_bpp,
  .get_bytes_per_line = arm_lcd_h3800_get_bytes_per_line,
  .get_info           = arm_lcd_h3800_get_info,
  .enable             = arm_lcd_h3800_enable,
  .disable            = arm_lcd_h3800_disable,
  .area_set           = arm_lcd_h3800_area_set,
  .area_put_pixel16   = arm_lcd_h3800_area_put_pixel16,
};
