...
//		result = netif_rx(skb);
                if (result != NET_RX_SUCCESS)
                {
                  if (result == NET_RX_DROP)
                    printk(KERN_WARNING "flips/ore: a packet was dropped by "
                                        "upper layers\n");
                  else
                    printk(KERN_WARNING "flips/ore: packet wasn't processed by"
                                        " upper layers, error: %d\n", result);
                }
