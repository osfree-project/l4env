...
#ifdef DDE_LINUX
/* We discovered several lockups with the FLIPS scheme that was: Use
 * __global_cli() on local_irq_disable(). So we decided to protect all accesses
 * to softnet_data[] with a special lock. Deadlocks cannot occur as
 * softnet_data_lock is always the inner lock. */
spinlock_t softnet_data_lock = SPIN_LOCK_UNLOCKED;
#endif
...
#else
	spin_lock_irqsave(&softnet_data_lock, flags);
#endif
...
#else
			spin_unlock_irqrestore(&softnet_data_lock, flags);
#endif
...
static void net_tx_action(struct softirq_action *h)
{
	...
	if (softnet_data[cpu].completion_queue) {
		...
		local_irq_disable();
#else
		spin_lock_irq(&softnet_data_lock);
#endif
		...
		local_irq_enable();
#else
		spin_unlock_irq(&softnet_data_lock);
#endif
