...
extern rwlock_t __brlock_array[NR_CPUS][__BR_END];

/* Link time error if index is out of bounds */
extern void __br_lock_usage_bug (void);
...
static inline void br_write_lock (enum brlock_indices idx)
{
	if (idx >= __BR_END)
		__br_lock_usage_bug();
	write_lock(&__brlock_array[smp_processor_id()][idx]);
}
...
static inline void br_write_unlock (enum brlock_indices idx)
{
	if (idx >= __BR_END)
		__br_lock_usage_bug();
	write_unlock(&__brlock_array[smp_processor_id()][idx]);
}
