...
typedef struct page {
	 ...
	 */
#if defined(DDE_LINUX) || defined(CONFIG_HIGHMEM) || defined(WANT_PAGE_VIRTUAL)
...
#else /* DDE_LINUX */
static inline struct page * alloc_pages(unsigned int gfp_mask,
                                        unsigned int order);
#endif /* DDE_LINUX */
                                        ...
                                        unsigned int order)
{
	...
	page = kmalloc(sizeof(*page), gfp_mask);

	memset(page, 0, sizeof(*page));

	page->virtual = (void *) __get_free_pages(gfp_mask, order);

	//printk("alloc_pages, page=%p page->virtual=%p\n", page, page->virtual);
	
	//increment reference counter of page
	get_page(page);
