...
struct fntfile_hdr {
	...
	u16	flags;			/* fontflags */
	u16	*hor_table;		/* pointer to horizontal table */
	u16	*off_table;		/* pointer to char offset table */
	u16	*dat_table;		/* pointer to font image */
	u16	form_width;		/* width of font image */
	u16	form_height;	/* height of font image */
	void	*next_font;		/* unused */
};

int init_conv_fnt(struct dope_services *d);


/*** ROUTINES TO CONVERT BIG/LITTLE ENDIAN DATA TO SYSTEM FORMAT ***/
...
static s16 font_probe(struct fntfile_hdr *fnt) {
	...
	u16 (*get_u16) (u16 *);
	DOPEDEBUG(char *dbg="ConvertFNT(probe): ";)
	
	if ((fnt->flags & 4)==0) {
		DOPEDEBUG(printf("%s font is in intel format\n",dbg));
		get_u16 = i2u16;
	} else  {
		DOPEDEBUG(printf("%s font is in motorola format\n",dbg));
		get_u16 = m2u16;
	}
	...
	img_w=8*get_u16(&fnt->form_width);
	...
	DOPEDEBUG(printf("%s image width:  %lu\n",dbg,img_w));
	DOPEDEBUG(printf("%s image height: %lu\n",dbg,img_h));
	DOPEDEBUG(printf("%s points:       %lu\n",dbg,point));
	DOPEDEBUG(printf("%s first_ade:    %lu\n",dbg,first_ade));
	DOPEDEBUG(printf("%s last_ade:     %lu\n",dbg,last_ade));
	DOPEDEBUG(printf("%s top:          %lu\n",dbg,top));
	DOPEDEBUG(printf("%s bottom:       %lu\n",dbg,bottom));
	DOPEDEBUG(printf("%s name:         %s\n",dbg,fnt->name));

	/* strange values -> seems not to be a valid fnt */
	if ((max_char_w>100)|(img_h<1)|(img_h>100)|(img_w<1)|(point>50)|
		(first_ade>255)|(last_ade>255)|(first_ade>last_ade)|
		(top>img_h)|(bottom>img_h)|(top<bottom)) {
		DOPEDEBUG(printf("%sthis doesnt seem to be valid fnt data - sorry\n",dbg));
		return 0;
	}
...
/* adress. It has one 32bit-entry per ASCII value -  */
/* so its size is 256*4 bytes. */
...
static u8 *font_get_name(struct fntfile_hdr *fnt) {
	fnt->name[31]=0;
...
static u16 font_get_top(struct fntfile_hdr *fnt) {
	if ((fnt->flags & 4)==0) return i2u16(&fnt->top);
	else return m2u16(&fnt->top);
}


/*** GET BOTTOM LINE OF FONT ***/
static u16 font_get_bottom(struct fntfile_hdr *fnt) {
	if ((fnt->flags & 4)==0) return i2u16(&fnt->bottom);
	else return m2u16(&fnt->bottom);
}


/*** GET HEIGHT OF FONT IMAGE ***/
static u32 font_get_image_width(struct fntfile_hdr *fnt) {
	if ((fnt->flags & 4)==0) return 8*i2u16(&fnt->form_width);
	else return 8*m2u16(&fnt->form_width);
}


/*** GET HEIGHT OF FONT IMAGE ***/
static u32 font_get_image_height(struct fntfile_hdr *fnt) {
	if ((fnt->flags & 4)==0) return i2u16(&fnt->form_height);
	else return m2u16(&fnt->form_height);
}


/*** GENERATES CHUNKY-ORGANIZED FONT IMAGE ***/
/* The image will be generated at the specified destination adress */
/* So the caller of this routine has to take care about memory allocation */
/* The size of a font image is image_width*image_height. */
static void font_gen_image(struct fntfile_hdr *fnt,u8 *dst) {
	...
	for (j=0;j<height;j++) {
		for (i=0;i<linelength;i++) {
			curr_byte=*(src++);
			for (k=0;k<8;k++) {
		 		if (((long)curr_byte)&0x0080) *(dst++)=255;
				else *(dst++)=0;
