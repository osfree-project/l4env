...
static void time_loop(void *data) {
	...
	while (1) {
		...
		dope_cmdf(app_id, "lb_time.set(-text \"uptime: %d:%.2d\")", s/60, s%60);
	}
}
...
void dope_loop(void *data) {
	...
	#include "window.i"
	dope_bind(app_id, "bt_reboot", "clack", bt_reboot_press_callback, NULL);
	dope_bind(app_id, "bt_clear", "click", bt_clear_press_callback, NULL);
	dope_cmd(app_id, "win_main.open()");

	dope_cmdf(app_id, "lb_mhz.set(-text \"%d MHz\")", l4_get_hz()/1000000);
